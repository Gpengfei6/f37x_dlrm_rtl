# Report the routed Vitis A14 design without modifying it.
# Required environment variables:
#   A14_ROUTED_DCP  routed checkpoint produced by v++ --link
#   A14_REPORT_DIR  new output directory for reports

proc fail {message} {
    error $message
}

foreach required_env {A14_ROUTED_DCP A14_REPORT_DIR} {
    if {![info exists ::env($required_env)] || $::env($required_env) eq ""} {
        fail "Missing required environment variable: $required_env"
    }
}

set dcp_path [file normalize $::env(A14_ROUTED_DCP)]
set report_dir [file normalize $::env(A14_REPORT_DIR)]
...
puts "A14_VITIS_POST_ROUTE_REPORT=COMPLETE"
