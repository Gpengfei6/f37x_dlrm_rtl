# Stage 2N-A17.6: package the unchanged A17 public kernel as a
...
if {![info exists ::env(A17_6_XO_DIR)]} { error "A17_6_XO_DIR is required; use the build runner" }
set build_dir [file normalize $::env(A17_6_XO_DIR)]
set project_dir [file join $build_dir vivado_project]
set ip_dir [file join $build_dir dlrm_f37x_stage2n_a17_ip_v1]
set top_name "dlrm_f37x_rtl_kernel_stage2n_a17_v1"
...
set rtl_files [list \
    ...
    [source_path $root_dir rtl/pipeline/dlrm_hbm_pipeline_integration_stage2n_a17_v1.sv] \
    [source_path $root_dir rtl/f37x/dlrm_f37x_rtl_kernel_stage2n_a17_v1.sv] \
]

puts "============================================================"
puts "Stage 2N-A17.6 A17 four-master target XO packaging v1"
puts "ROOT=$root_dir"
...
if {[file exists $build_dir]} {
    fail "Refusing to overwrite existing A17.6 XO build directory: $build_dir"
}
file mkdir $build_dir

create_project dlrm_f37x_stage2n_a17_package_v1 $project_dir \
...
set_property display_name \
    {F37X A17.6 Four-Master HBM Lookup RTL Kernel} $core
set_property description \
    {A17 user-managed wrapper with four 64-bit-address 128-bit-data AXI masters and four runtime table-base pointers} $core
set_property version 2.176 $core
...
if {[llength [ipx::get_registers -of_objects $address_block]] != 4} {
    fail "A17.6 requires exactly four pointer arguments"
}
...
puts "============================================================"
puts "A17_6_TARGET_XO_PACKAGE=PASS"
puts "KERNEL=$top_name"
puts "TARGET_PART=$target_part_name"
puts "PACKAGING_PART=$part_name"
puts "TARGET_PART_USED=$target_part_used"
