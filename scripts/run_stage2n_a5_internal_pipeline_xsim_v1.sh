#!/usr/bin/env bash
#
# Stage 2N-A5 internal Bottom-MLP -> Interaction -> Top-MLP XSim regression.
#
# Simulation only:
#   * no XO packaging
#   * no xclbin link
...
WORK_DIR="${REPO_ROOT}/work/stage2n_a5_internal_pipeline_xsim_v1"
LOG_DIR="${REPO_ROOT}/logs"
RESULT_DIR="${REPO_ROOT}/results/stage2n_a5"
STATUS_FILE="${RESULT_DIR}/stage2n_a5_internal_pipeline_xsim_v1_status.txt"

XVLOG_LOG="${LOG_DIR}/xvlog_stage2n_a5_internal_pipeline_v1.log"
XELAB_LOG="${LOG_DIR}/xelab_stage2n_a5_internal_pipeline_v1.log"
XSIM_LOG="${LOG_DIR}/xsim_stage2n_a5_internal_pipeline_v1.log"

TOP_MODULE="tb_dlrm_internal_pipeline_controller_v1"
SNAPSHOT="tb_dlrm_internal_pipeline_controller_v1_sim"
...
SOURCE_SEGMENTED_MLP="${REPO_ROOT}/rtl/control/mlp_sequence_controller_segmented.sv"
SOURCE_INTERACTION="$(find_tracked_unique dlrm_feature_interaction_engine.sv)"
SOURCE_PIPELINE="${REPO_ROOT}/rtl/pipeline/dlrm_internal_pipeline_controller.sv"
SOURCE_TB="${REPO_ROOT}/tb/tb_dlrm_internal_pipeline_controller_v1.sv"
...
echo "============================================================"
echo "Stage 2N-A5 Internal DLRM Pipeline XSim v1"
echo "TIME=$(date -Is)"
echo "REPO=${REPO_ROOT}"
...
PASS_MARKER="tb_dlrm_internal_pipeline_controller_v1: PASS runs=2 bottom=8 interaction=18 top=-60 backpressure=12"

grep -qF "${PASS_MARKER}" "${XSIM_LOG}" ||
    fail "Stage 2N-A5 PASS marker is missing"
...
if grep -E '^(ERROR:|FATAL:)' \
    ...
    "${XSIM_LOG}"
then
    fail "Stage 2N-A5 logs contain anchored errors"
fi

{
    echo "STAGE2N_A5_INTERNAL_PIPELINE_XSIM_V1_PASS"
    echo "TIME=$(date -Is)"
    echo "BRANCH=${GIT_BRANCH}"
    echo "HEAD=${GIT_HEAD}"
    echo "TOP=${TOP_MODULE}"
    echo "BOTTOM_DESCRIPTOR_BASE=0"
    echo "BOTTOM_LAYER_COUNT=1"
    echo "BOTTOM_OUTPUT_DIM=8"
    echo "INTERACTION_VECTOR_COUNT=5"
    echo "INTERACTION_VECTOR_DIM=8"
    echo "INTERACTION_OUTPUT_DIM=18"
    echo "TOP_DESCRIPTOR_BASE=1"
    echo "TOP_LAYER_COUNT=1"
    echo "TOP_OUTPUT_DIM=1"
    echo "TOP_RESULT=-60"
    echo "PIPELINE_RUNS=2"
    echo "FINAL_RESULT_BACKPRESSURE_CYCLES=12"
    echo "ONE_DENSE_ENGINE=1"
    echo "HOST_STARTS_ONLY_FULL_PIPELINE=1"
    echo "NO_AXI_WRAPPER_IN_A5=1"
    echo "NO_FPGA_PROGRAMMING_OR_RESET=1"
    echo "SEGMENTED_MLP_SHA256=$(sha256sum "${SOURCE_SEGMENTED_MLP}" | awk '{print $1}')"
    echo "PIPELINE_SHA256=$(sha256sum "${SOURCE_PIPELINE}" | awk '{print $1}')"
    echo "TB_SHA256=$(sha256sum "${SOURCE_TB}" | awk '{print $1}')"
    echo "RUNNER_SHA256=$(sha256sum "${REPO_ROOT}/scripts/run_stage2n_a5_internal_pipeline_xsim_v1.sh" | awk '{print $1}')"
    echo "LOG=${XSIM_LOG}"
} > "${STATUS_FILE}"
