#!/usr/bin/env bash
#
# Stage 2K-A: directed scalar and repeated-stability regression on F37X.
#
# Locked target:
#   xbutil index : 2
#   BDF          : 0000:9b:00.1
...
SOURCE="host/stage2k_a_scalar_regression_hal.cpp"
BUILD_DIR="build/stage2k"
BINARY="${BUILD_DIR}/stage2k_a_scalar_regression_hal"
...
CSV="${RESULT_DIR}/stage2k_a_scalar_regression_${STAMP}.csv"
LOG="${SERVER_RESULT_DIR}/stage2k_a_scalar_regression_${STAMP}.log"
STATUS="${RESULT_DIR}/stage2k_a_scalar_regression_status.txt"
...
write_failure() {
    local reason="$1"
    {
        echo "STAGE2K_A_SCALAR_REGRESSION_FAILED"
        echo "TIME=$(date -Is)"
...
(( STABILITY_REPETITIONS >= 1 &&
   STABILITY_REPETITIONS <= 10000 )) ||
    fail "stability repetitions must be in 1..10000"

echo "============================================================"
echo "Stage 2K-A F37X DLRM Scalar Regression"
echo "TIME=$(date -Is)"
echo "REPO=$(pwd)"
...
[[ -f "${SOURCE}" ]] ||
    fail "Stage 2K-A host source not found: ${SOURCE}"
[[ -s "${XCLBIN}" ]] ||
...
[[ "${BUILD_RC}" -eq 0 ]] ||
    fail "Stage 2K-A host compilation returned ${BUILD_RC}"

ls -lh "${BINARY}"

echo
echo "========== RUN STAGE 2K-A HOST =========="
set +e
...
[[ "${HOST_RC}" -eq 0 ]] ||
    fail "Stage 2K-A host executable returned ${HOST_RC}"

[[ -s "${CSV}" ]] ||
    fail "Stage 2K-A CSV was not created"

EXPECTED_ROWS=$((9 + STABILITY_REPETITIONS))
...
[[ "${ACTUAL_ROWS}" -eq "${EXPECTED_ROWS}" ]] ||
    fail "CSV row count mismatch: expected ${EXPECTED_ROWS}, got ${ACTUAL_ROWS}"
[[ "${PASS_ROWS}" -eq "${EXPECTED_ROWS}" ]] ||
    fail "CSV PASS count mismatch: expected ${EXPECTED_ROWS}, got ${PASS_ROWS}"
...
[[ "${POST_UUID}" == "${EXPECTED_UUID}" ]] ||
    fail "xclbin UUID changed after Stage 2K-A: ${POST_UUID}"
echo "${POST_QUERY}" | grep -q "Level 0 : 0x0(GOOD)" ||
    fail "target firewall is not GOOD after Stage 2K-A"
echo "${POST_QUERY}" | grep -q "${EXPECTED_CU}" ||
    fail "DLRM compute unit disappeared after Stage 2K-A"
echo "${POST_QUERY}" | grep -q "(IDLE)" ||
    fail "DLRM compute unit is not IDLE after Stage 2K-A"

grep -q "STAGE2K_A_SCALAR_REGRESSION_PASS" "${LOG}" ||
    fail "Stage 2K-A PASS marker missing from log"

{
    echo "STAGE2K_A_SCALAR_REGRESSION_PASS"
    echo "TIME=$(date -Is)"
    echo "TARGET_INDEX=${TARGET_INDEX}"
    echo "TARGET_BDF=${TARGET_BDF}"
    echo "TARGET_RENDER=${TARGET_RENDER}"
    ...
    echo "IP_INDEX=0"
    echo "ACCESS=xclOpenContext+xclRegRead+xclRegWrite"
    echo "DIRECTED_CASES=9"
    echo "STABILITY_CASES=${STABILITY_REPETITIONS}"
    echo "TOTAL_CASES=${EXPECTED_ROWS}"
