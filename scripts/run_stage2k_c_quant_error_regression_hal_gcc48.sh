#!/usr/bin/env bash
#
# Stage 2K-C: quantization, saturation, validation-error, and recovery
# regression on the locked F37X board.
#
# Locked target:
#   xbutil index : 2
#   BDF          : 0000:9b:00.1
...
ERROR_CASES="8"
RECOVERY_PROBES="8"
EXPECTED_RESULT_ROWS=$((POSITIVE_CASES + RECOVERY_PROBES))
EXPECTED_RECORDS=$((POSITIVE_CASES + ERROR_CASES + RECOVERY_PROBES))

XCLBIN="build/stage2h/hw/dlrm_f37x_rtl_kernel.xclbin"
SOURCE="host/stage2k_c_quant_error_regression_hal.cpp"
BUILD_DIR="build/stage2k"
BINARY="${BUILD_DIR}/stage2k_c_quant_error_regression_hal"
...
CSV="${RESULT_DIR}/stage2k_c_quant_error_regression_${STAMP}.csv"
LOG="${SERVER_RESULT_DIR}/stage2k_c_quant_error_regression_${STAMP}.log"
STATUS="${RESULT_DIR}/stage2k_c_quant_error_regression_status.txt"
...
write_failure() {
    local reason="$1"
    {
        echo "STAGE2K_C_QUANT_ERROR_REGRESSION_FAILED"
        echo "TIME=$(date -Is)"
...
echo "============================================================"
echo "Stage 2K-C F37X Quantization and Error Regression"
echo "TIME=$(date -Is)"
echo "REPO=$(pwd)"
...
echo "TARGET_BDF=${TARGET_BDF}"
echo "TARGET_RENDER=${TARGET_RENDER}"
echo "POSITIVE_CASES=${POSITIVE_CASES}"
echo "ERROR_CASES=${ERROR_CASES}"
echo "RECOVERY_PROBES=${RECOVERY_PROBES}"
...
[[ -f "${SOURCE}" ]] ||
    fail "Stage 2K-C host source not found: ${SOURCE}"
[[ -s "${XCLBIN}" ]] ||
...
[[ "${BUILD_RC}" -eq 0 ]] ||
    fail "Stage 2K-C host compilation returned ${BUILD_RC}"

ls -lh "${BINARY}"

echo
echo "========== RUN STAGE 2K-C HOST =========="
set +e
"${BINARY}" "${CSV}"
HOST_RC=$?
set -e
...
[[ "${HOST_RC}" -eq 0 ]] ||
    fail "Stage 2K-C host executable returned ${HOST_RC}"

[[ -s "${CSV}" ]] ||
    fail "Stage 2K-C CSV was not created"

ACTUAL_ROWS="$(awk 'NR > 1 {count++} END {print count+0}' "${CSV}")"
PASS_ROWS="$(awk -F, 'NR > 1 && $NF == "PASS" {count++} END {print count+0}' "${CSV}")"
RESULT_ROWS="$(awk -F, 'NR > 1 && $4 == "result" {count++} END {print count+0}' "${CSV}")"
ERROR_ROWS="$(awk -F, 'NR > 1 && $4 == "error" {count++} END {print count+0}' "${CSV}")"
RESULT_MISMATCH_ROWS="$(awk -F, 'NR > 1 && $4 == "result" && $9 != $10 {count++} END {print count+0}' "${CSV}")"
ERROR_MISMATCH_ROWS="$(awk -F, 'NR > 1 && $4 == "error" && $11 != $12 {count++} END {print count+0}' "${CSV}")"

[[ "${ACTUAL_ROWS}" -eq "${EXPECTED_RECORDS}" ]] ||
    fail "CSV row mismatch: expected ${EXPECTED_RECORDS}, got ${ACTUAL_ROWS}"
[[ "${PASS_ROWS}" -eq "${EXPECTED_RECORDS}" ]] ||
    fail "CSV PASS count mismatch: expected ${EXPECTED_RECORDS}, got ${PASS_ROWS}"
[[ "${RESULT_ROWS}" -eq "${EXPECTED_RESULT_ROWS}" ]] ||
    fail "CSV result-row mismatch: expected ${EXPECTED_RESULT_ROWS}, got ${RESULT_ROWS}"
[[ "${ERROR_ROWS}" -eq "${ERROR_CASES}" ]] ||
    fail "CSV error-row mismatch: expected ${ERROR_CASES}, got ${ERROR_ROWS}"
[[ "${RESULT_MISMATCH_ROWS}" -eq 0 ]] ||
    fail "CSV contains ${RESULT_MISMATCH_ROWS} result mismatches"
[[ "${ERROR_MISMATCH_ROWS}" -eq 0 ]] ||
    fail "CSV contains ${ERROR_MISMATCH_ROWS} error-code mismatches"
...
[[ "${POST_UUID}" == "${EXPECTED_UUID}" ]] ||
    fail "xclbin UUID changed after Stage 2K-C: ${POST_UUID}"
echo "${POST_QUERY}" | grep -q "Level 0 : 0x0(GOOD)" ||
    fail "target firewall is not GOOD after Stage 2K-C"
echo "${POST_QUERY}" | grep -q "${EXPECTED_CU}" ||
    fail "DLRM compute unit disappeared after Stage 2K-C"
echo "${POST_QUERY}" | grep -q "(IDLE)" ||
    fail "DLRM compute unit is not IDLE after Stage 2K-C"

grep -q "STAGE2K_C_QUANT_ERROR_REGRESSION_PASS" "${LOG}" ||
    fail "Stage 2K-C PASS marker missing from log"

{
    echo "STAGE2K_C_QUANT_ERROR_REGRESSION_PASS"
    echo "TIME=$(date -Is)"
    echo "TARGET_INDEX=${TARGET_INDEX}"
    echo "TARGET_BDF=${TARGET_BDF}"
    echo "TARGET_RENDER=${TARGET_RENDER}"
    ...
    echo "ERROR_CASES=${ERROR_CASES}"
    echo "RECOVERY_PROBES=${RECOVERY_PROBES}"
    echo "TOTAL_RECORDS=${EXPECTED_RECORDS}"
    echo "CSV_ROWS=${ACTUAL_ROWS}"
    echo "CSV_PASS_ROWS=${PASS_ROWS}"
    echo "CSV_RESULT_ROWS=${RESULT_ROWS}"
    echo "CSV_ERROR_ROWS=${ERROR_ROWS}"
    echo "CSV_RESULT_MISMATCH_ROWS=${RESULT_MISMATCH_ROWS}"
    echo "CSV_ERROR_MISMATCH_ROWS=${ERROR_MISMATCH_ROWS}"
    echo "MISSING_DESCRIPTOR_BOUNDARY=not_testable_without_clearing_persistent_descriptor_table"
