#!/usr/bin/env bash
# Build-only XRT 2020.2 gate for the A17.6 four-BO Host.
...
SOURCE="${REPO_ROOT}/host/stage2n_a17_6_four_bo_host_v1.cpp"
BUILD_DIR="${REPO_ROOT}/build/stage2n_a17_6/host_v1"
BINARY="${BUILD_DIR}/stage2n_a17_6_four_bo_host_v1"
...
{
    local code="$1"; shift
    echo "A17_6_HOST_XRT_BUILD=FAIL" >&2
    echo "REASON=$*" >&2
    exit "${code}"
}

[[ -s "${SOURCE}" ]] || fail 3 "A17.6 Host source is missing"
[[ -f "${XRT_SETUP}" ]] || fail 4 "XRT setup is missing: ${XRT_SETUP}"
if [[ -e "${BUILD_DIR}" ]]; then
    [[ "${A17_6_ALLOW_HOST_REBUILD:-no}" == "yes" ]] ||
        fail 2 "Host build directory exists; set A17_6_ALLOW_HOST_REBUILD=yes only to rebuild this Host"
...
set -e
[[ "${build_exit}" -eq 0 ]] || fail "${build_exit}" "A17.6 Host compile/link failed"
...
A17_6_XRT2020_2_API_PROBE=PASS
...
EOF
echo "A17_6_HOST_XRT_BUILD=PASS"
echo "A17_6_XRT2020_2_API_PROBE=PASS"
