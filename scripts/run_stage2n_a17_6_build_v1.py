#!/usr/bin/env python3
"""A17.6 source bundle / user-executed XO / link. No device or Host operations.

Target commands require an already initialized Vitis 2020.2 shell and explicit
--confirm-build. Codex must not execute those subcommands on the target.
"""
...
def xo_paths(directory):
    return (directory / "xo" / (KERNEL + ".xo"),
            ...
            directory / "xo/dlrm_f37x_stage2n_a17_ip_v1/component.xml")
...
def execute(args):
    ...
    if args.phase == "check":
        print("A17_6_SOURCE_INTEGRITY=PASS")
    ...
    try:
        ...
        env = os.environ.copy()
        if args.phase == "xo":
            env["A17_6_XO_DIR"] = str(output / "xo")
        ...
        write_json(status_path, status)
    print("A17_6_{}=PASS".format(args.phase.upper()))
...
def main():
    ...
    try:
        ...
        print("A17_6_{}=FAIL: {}".format(args.phase.upper(), error), file=sys.stderr)
        return 1
    return 0
