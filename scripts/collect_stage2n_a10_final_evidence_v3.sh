#!/usr/bin/env bash
#
# Collect and validate final Stage 2N-A10 evidence (v3 path fix).
#
...
EVIDENCE_DIR="${REPO_ROOT}/docs/evidence/stage2n_a10_final_v3"
FINAL_STATUS="${EVIDENCE_DIR}/stage2n_a10_final_acceptance_v3_status.txt"
MANIFEST="${EVIDENCE_DIR}/stage2n_a10_final_evidence_manifest_v3.sha256"
...
STAGE2N_A10_FINAL_ACCEPTANCE_V3_FAILED
REASON=${reason}
NO_FPGA_ACCESS=1
...
require_exact "${BOARD_STATUS}" "TARGET=0000:9b:00.1 (xbutil index 2)"
require_exact "${BOARD_STATUS}" "UUID=${EXPECTED_UUID}"
require_exact "${BOARD_STATUS}" "CU=${EXPECTED_CU}"
...
require_exact "${BOARD_STATUS}" "NO FPGA RESET WAS PERFORMED"
require_exact "${BOARD_STATUS}" "NO OTHER DEVICE WAS ACCESSED"

# Host-level evidence inside the complete log.
for line in \
    "PIPELINE_STALE_RESULT_DETECTED=1" \
    "PIPELINE_STALE_RESULT=36" \
    "PIPELINE_STALE_TAG=4" \
    "PIPELINE_STALE_RESULT_POPPED=1" \
    "PIPELINE_RUN[0]_RESULT=36" \
    "PIPELINE_RUN[0]_TAG=4" \
    "PIPELINE_RUN[0]_EXPECTED_FINAL_DESCRIPTOR_TAG=4" \
    "PIPELINE_RUN[1]_RESULT=36" \
    "PIPELINE_RUN[1]_TAG=4" \
    "PIPELINE_RUN[1]_EXPECTED_FINAL_DESCRIPTOR_TAG=4" \
...
cat > "${FINAL_STATUS}" <<EOF
STAGE2N_A10_FINAL_ACCEPTANCE_V3_PASS
...
echo "============================================================"
echo "STAGE2N_A10_FINAL_ACCEPTANCE_V3_PASS"
echo "EVIDENCE_COLLECTOR_VERSION=3"
