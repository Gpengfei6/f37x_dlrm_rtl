...
BUILD_DIR="${REPO_ROOT}/build/stage2n_a3_v3"
OUTPUT_DIR="${BUILD_DIR}/hw"
TEMP_DIR="${BUILD_DIR}/_x"
XCLBIN_PATH="${OUTPUT_DIR}/${KERNEL_NAME}.xclbin"

LOG_DIR="${REPO_ROOT}/logs"
RESULT_DIR="${REPO_ROOT}/results/stage2n_a3_v3"

VPP_LOG="${LOG_DIR}/vpp_stage2n_a3_f37x_link_v3.log"
XCLBIN_INFO="${RESULT_DIR}/${KERNEL_NAME}.xclbin.info"
STATUS_FILE="${RESULT_DIR}/stage2n_a3_f37x_link_v3_status.txt"
...
write_failure_status()
{
    ...
    cat > "${STATUS_FILE}" <<EOF
STAGE2N_A3_F37X_LINK_V3_FAILED
REASON=${reason}
EXIT_CODE=${exit_code}
KERNEL=${KERNEL_NAME}
...
if (( KERNEL_CU_SPEC_LENGTH > 64 )); then
    fail 1 "kernel/CU specification is ${KERNEL_CU_SPEC_LENGTH} characters; Vitis 2020.2 limit is 64"
fi
...
echo "============================================================"
echo "Stage 2N-A3 v3 F37X hardware link"
...
echo "Kernel:CU length     : ${KERNEL_CU_SPEC_LENGTH}/64"
...
if [[ "${VPP_EXIT_CODE}" != "0" ]]; then
    fail "${VPP_EXIT_CODE}" "v++ hardware link failed; inspect ${VPP_LOG}"
fi
...
CRITICAL_WARNING_COUNT="$(grep -cE '^CRITICAL WARNING:' "${VPP_LOG}" || true)"

if [[ "${ERROR_COUNT}" != "0" ]]; then
    fail 18 "v++ log contains ${ERROR_COUNT} anchored errors"
fi
if [[ "${CRITICAL_WARNING_COUNT}" != "0" ]]; then
    fail 19 "v++ log contains ${CRITICAL_WARNING_COUNT} critical warnings"
fi
...
grep -qF "Instance:        ${CU_NAME}" "${XCLBIN_INFO}" ||
    fail 21 "xclbin metadata does not contain compute unit ${CU_NAME}"

grep -qF 'Platform VBNV:          inspur_f37x_xdma_201920_3' "${XCLBIN_INFO}" ||
    fail 22 "xclbin metadata does not contain the expected F37X platform"
...
echo "============================================================"
echo "STAGE2N_A3_F37X_LINK_V3_PASS"
echo "XCLBIN=${XCLBIN_PATH}"
