# Stage 2N-A16.2: package the accepted A16.1 instrumented public kernel as a
...
set build_dir [file join $root_dir build stage2n_a16_2 xo_v1]
set project_dir [file join $build_dir vivado_project]
set ip_dir [file join $build_dir dlrm_f37x_stage2n_a16_ip_v1]
set top_name "dlrm_f37x_rtl_kernel_stage2n_a16_v1"
set target_part_name "xcvu37p-fsvh2892-2L-e"
set part_name $target_part_name
if {$argc > 1} {
    error "Usage: vivado -source package_stage2n_a16_2_rtl_kernel_v1.tcl ?packaging_part?"
}
if {$argc == 1} {
...
puts "============================================================"
puts "Stage 2N-A16.2 accepted A16.1 target XO packaging v1"
puts "ROOT=$root_dir"
...
if {[file exists $build_dir]} {
    fail "Refusing to overwrite existing A16.2 XO build directory: $build_dir"
}
file mkdir $build_dir

create_project dlrm_f37x_stage2n_a16_package_v1 $project_dir \
...
set_property display_name \
    {F37X A16.2 Sequential HBM Latency Instrumented RTL Kernel} $core
set_property description \
    {Accepted A16.1 user-managed wrapper with one 64-bit-address 128-bit-data m_axi_gmem read master, a 64-bit TABLE_BASE pointer and read-only latency counters} $core
set_property version 2.18 $core
...
# The complete A13/A15/A16 register ABI is intentionally accessed through raw
...
if {[llength $all_registers] != 1} {
    fail "A16.2 package metadata must expose exactly TABLE_BASE as an argument"
}
...
puts "============================================================"
puts "A16_2_TARGET_XO_PACKAGE=PASS"
puts "KERNEL=$top_name"
puts "TARGET_PART=$target_part_name"
puts "PACKAGING_PART=$part_name"
puts "TARGET_PART_USED=$target_part_used"
