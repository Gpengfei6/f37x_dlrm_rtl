#!/usr/bin/env python3
"""Offline A17.6 artifact checks. A successful check is never a board PASS."""
...
PACKAGE = "scripts/package_stage2n_a17_6_rtl_kernel_v1.tcl"
CONFIG = "config/stage2n_a17_6_target_v1.cfg"
REPORT = "scripts/report_stage2n_a16_2_vitis_post_route_v1.tcl"
MANIFEST = "config/stage2n_a17_6_sources_v1.json"
...
def source_files(root):
    ...
    require(len(sources) == 20 and len(set(sources)) == 20, "expected 20 unique RTL sources")
    return sources
...
def config_check(path):
    ...
    require(actual == expected, "connectivity must contain one A17 CU and exact four-bank map")


def source_check(root):
    manifest = json.loads((root / MANIFEST).read_text(encoding="utf-8"))
    require(manifest.get("schema") == "a17_6_source_manifest_v1", "source manifest schema mismatch")
    ...
    expected = set(source_files(root) + [PACKAGE, CONFIG, REPORT,
        ...
        "scripts/validate_stage2n_a17_6_artifacts_v1.py",
        "scripts/run_stage2n_a17_6_build_v1.py",
        "scripts/test_stage2n_a17_6_build_v1.py",
        "docs/STAGE2N_A17_6_BUILD_HANDOFF_V1.md"])
...
def component_check(data):
    ...
    require(parse_scaled_integer(child_text(block, "range", "range")) >= 0x330, "control range too small")
