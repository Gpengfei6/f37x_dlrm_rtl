#!/usr/bin/env bash
# Build-only XRT 2020.2 gate for the A18.2 four-BO Host.
...
SOURCE="${REPO_ROOT}/host/stage2n_a18_2_four_bo_host_v1.cpp"
BUILD_DIR="${REPO_ROOT}/build/stage2n_a18_2/host_v1"
BINARY="${BUILD_DIR}/stage2n_a18_2_four_bo_host_v1"
...
{
    local code="$1"; shift
    echo "A18_2_HOST_XRT_BUILD=FAIL" >&2
    echo "REASON=$*" >&2
    exit "${code}"
}

[[ -s "${SOURCE}" ]] || fail 3 "A18.2 Host source is missing"
[[ -f "${XRT_SETUP}" ]] || fail 4 "XRT setup is missing: ${XRT_SETUP}"
if [[ -e "${BUILD_DIR}" ]]; then
    [[ "${A18_2_ALLOW_HOST_REBUILD:-no}" == "yes" ]] ||
        fail 2 "Host build directory exists; set A18_2_ALLOW_HOST_REBUILD=yes only to rebuild this Host"
...
set -e
[[ "${build_exit}" -eq 0 ]] || fail "${build_exit}" "A18.2 Host compile/link failed"
...
cat > "${STATUS}" <<EOF
A18_2_HOST_XRT_BUILD=PASS
A18_2_XRT2020_2_API_PROBE=PASS
...
EOF
echo "A18_2_HOST_XRT_BUILD=PASS"
echo "A18_2_XRT2020_2_API_PROBE=PASS"
