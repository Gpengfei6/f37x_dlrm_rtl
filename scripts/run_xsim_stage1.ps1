...
if ([string]::IsNullOrWhiteSpace($SummaryPath)) {
  $SummaryPath = Join-Path $ProjectRoot "results/xsim_stage1_summary.json"
}
...
if ($Missing.Count -gt 0) {
  ...
  Write-Output "run_xsim_stage1: SKIPPED - $($Summary.reason)"
  exit 2
}

$DriverLog = Join-Path $ProjectRoot "logs/xsim_driver.log"
$Output = & $VivadoPath -mode batch -nojournal -nolog `
  -source "scripts/run_xsim_stage1.tcl" 2>&1
$ExitCode = $LASTEXITCODE
$Output | Out-File -LiteralPath $DriverLog -Encoding utf8
$Output | ForEach-Object { Write-Output $_ }

$Tests = @()
$StatusPath = Join-Path $ProjectRoot "results/xsim_stage1_status.txt"
if (Test-Path -LiteralPath $StatusPath) {
  foreach ($Line in Get-Content -LiteralPath $StatusPath) {
    ...
    $Stage = $Parts[1]
    $RawStatus = $Parts[2]
    $CommandExitCode = [int]$Parts[3]
    $Status = if ($RawStatus -eq "PASS") { "PASS" } else { "FAIL" }
    $Tests += [pscustomobject]@{
      name = $Parts[0]; stage = $Stage; status = $Status
      exit_code = $CommandExitCode; detail = $RawStatus
    }
  }
}
...
$Summary = [ordered]@{
  status = $Overall; exit_code = $ExitCode; log = "logs/xsim_driver.log"
  tests = $Tests
}
[System.IO.File]::WriteAllText($SummaryPath,
  (($Summary | ConvertTo-Json -Depth 6) + [Environment]::NewLine),
  [System.Text.UTF8Encoding]::new($false))
Write-Output "run_xsim_stage1: $Overall"
