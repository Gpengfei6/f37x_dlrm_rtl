#!/usr/bin/env bash
#
# Stage 2N-A10 v3 protected F37X board smoke for the capacity-expanded five-layer pipeline.
#
...
SOURCE="host/stage2n_a10_pipeline_board_smoke_v3.cpp"
BUILD_DIR="build/stage2n_a10_board_v3"
RESULT_DIR="results/stage2n_a10_board_v3"
SERVER_RESULT_DIR="server_results/stage2n_a10"

mkdir -p "${BUILD_DIR}" "${RESULT_DIR}" "${SERVER_RESULT_DIR}"

STAMP="$(date +%Y%m%d_%H%M%S)"
BINARY="${BUILD_DIR}/stage2n_a10_pipeline_board_smoke_v3_${STAMP}"
LOG="${SERVER_RESULT_DIR}/stage2n_a10_pipeline_board_smoke_v3_${STAMP}.log"
STATUS="${RESULT_DIR}/stage2n_a10_pipeline_board_smoke_v3_status_${STAMP}.txt"
...
write_failure()
{
    ...
    {
        echo "STAGE2N_A10_PIPELINE_BOARD_SMOKE_V3_FAILED"
        echo "TIME=$(date -Is)"
...
echo "============================================================"
echo "Stage 2N-A10 v3 Protected F37X Five-Layer Pipeline Board Smoke"
echo "TIME=$(date -Is)"
echo "REPO=$(pwd)"
...
grep -qF \
    "STAGE2N_A10_PIPELINE_BOARD_SMOKE_V3_PASS runs=2 final=36 bottom=8 interaction=18 descriptors=5 weights=1360 biases=73" \
    "${LOG}" ||
...
{
    echo "STAGE2N_A10_PIPELINE_BOARD_SMOKE_V3_PASS"
    echo "TIME=$(date -Is)"
    echo "TARGET_INDEX=${TARGET_INDEX}"
    echo "TARGET_BDF=${TARGET_BDF}"
    echo "TARGET_RENDER=${TARGET_RENDER}"
