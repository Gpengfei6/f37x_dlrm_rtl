...
$baselineCommit = '876046c3b27e2c88b36e394d990fff683bda3ad5'
$top = 'tb_dlrm_hbm_pipeline_integration_stage2n_a15_v2'
$snapshot = 'tb_dlrm_hbm_pipeline_integration_stage2n_a15_v2_sim'
$runtimeTcl = Join-Path $scriptDir `
    'run_stage2n_a15_2_pipeline_xsim_v1.tcl'
...
$sources = @(
    ...
    (Join-Path $repoRoot `
        'tb\tb_dlrm_hbm_pipeline_integration_stage2n_a15_v2.sv')
)
...
function Fail([string]$Message) {
    throw "Stage 2N-A15.2 XSim failed: $Message"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Fail "baseline commit is not an ancestor of HEAD"
    }
    ...
    @(
        'STAGE2N_A15_2_END_TO_END_PIPELINE_XSIM=RUNNING'
        "BRANCH=$branch"
        "HEAD=$head"
    ...
    try {
        ...
        try {
            Write-Output "A15_2_XSIM_TOP=$top"
            Write-Output "A15_2_XSIM_SOURCE_COUNT=$($sources.Count)"

            & $xvlog --sv @sources 2>&1 |
                Tee-Object -LiteralPath `
                    (Join-Path $ResultDir 'a15_2_xvlog.log') |
                Out-Host
            $xvlogExit = $LASTEXITCODE
            Write-Output "A15_2_XVLOG_EXIT=$xvlogExit"
            if ($xvlogExit -ne 0) {
            ...
            & $xelab $top -s $snapshot --timescale 1ns/1ps 2>&1 |
                Tee-Object -LiteralPath `
                    (Join-Path $ResultDir 'a15_2_xelab.log') |
                Out-Host
            $xelabExit = $LASTEXITCODE
            Write-Output "A15_2_XELAB_EXIT=$xelabExit"
            if ($xelabExit -ne 0) {
            ...
            & $xsim $snapshot -tclbatch $runtimeTclForXsim 2>&1 |
                Tee-Object -LiteralPath `
                    (Join-Path $ResultDir 'a15_2_xsim.log') |
                Out-Host
            $xsimExit = $LASTEXITCODE
            Write-Output "A15_2_XSIM_EXIT=$xsimExit"
            if ($xsimExit -ne 0) {
    ...
    $xsimLog = Join-Path $ResultDir 'a15_2_xsim.log'
    ...
    $errors = @($primaryLogs | ForEach-Object {
        Select-String -LiteralPath $_ `
            -Pattern '^\s*(?:ERROR\s*:|FATAL\s*:|Fatal\s:)'
    })
    if ($errors.Count -ne 0) {
        Fail "anchored error/fatal count is $($errors.Count)"
    }

    $expectedResult = Read-UniqueMarker $xsimLog 'EXPECTED_RESULT'
    $finalResult = Read-UniqueMarker $xsimLog 'FINAL_RESULT'
    ...
    $overheadCycles = Read-UniqueMarker $xsimLog `
        'CONTROLLER_OVERHEAD_CYCLES'

    if ($expectedResult -ne '36' -or $finalResult -ne '36') {
        Fail "golden/result marker mismatch expected=$expectedResult actual=$finalResult"
    }
    if ($bottomCycles -ne '322' -or $interactionCycles -ne '100' -or
        $topCycles -ne '744' -or $totalCycles -ne '1174' -or
        $overheadCycles -ne '8') {
        Fail 'accepted A13 cycle marker mismatch'
    }

    @(
        'STAGE2N_A15_2_END_TO_END_PIPELINE_XSIM=PASS'
        "BRANCH=$branch"
        "HEAD=$head"
        "BASELINE_COMMIT=$baselineCommit"
        "TOP=$top"
    ...
    if ($resultRootCreated) {
        @(
            'STAGE2N_A15_2_END_TO_END_PIPELINE_XSIM=FAIL'
            "BRANCH=$branch"
            "HEAD=$head"
