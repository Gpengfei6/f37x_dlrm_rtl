#!/usr/bin/env bash
#
# Stage 2N-A10 v1 F37X hardware link for the capacity-expanded automatic pipeline.
#
# Consumes the verified A10 v2 XO and generates an F37X hardware xclbin.
...
PACKAGE_DIR="${REPO_ROOT}/build/stage2n_a10/package_v2"
XO_PATH="${PACKAGE_DIR}/${KERNEL_NAME}.xo"
KERNEL_XML="${PACKAGE_DIR}/kernel.xml"

BUILD_DIR="${REPO_ROOT}/build/stage2n_a10/link_v1"
...
VPP_LOG="${LOG_DIR}/vpp_stage2n_a10_f37x_link_v1.log"
XCLBIN_INFO="${RESULT_DIR}/${KERNEL_NAME}.xclbin.info"
STATUS_FILE="${RESULT_DIR}/stage2n_a10_f37x_link_v1_status.txt"
...
fail()
{
    ...
    cat > "${STATUS_FILE}" <<EOF
STAGE2N_A10_F37X_LINK_V1_FAILED
REASON=${reason}
EXIT_CODE=${code}
KERNEL=${KERNEL_NAME}
...
[[ -s "${XO_PATH}" ]] ||
    fail 8 "A10 v2 XO not found or empty: ${XO_PATH}"

[[ -s "${KERNEL_XML}" ]] ||
    fail 9 "A10 v2 kernel.xml not found or empty: ${KERNEL_XML}"

PACKAGE_STATUS="${REPO_ROOT}/results/stage2n_a10/package_v2/stage2n_a10_xo_package_v2_status.txt"

[[ -s "${PACKAGE_STATUS}" ]] ||
    fail 9 "A10 v2 XO package status is missing or empty: ${PACKAGE_STATUS}"

grep -Fxq "STAGE2N_A10_XO_PACKAGE_V2_PASS" "${PACKAGE_STATUS}" ||
    fail 9 "A10 v2 XO package status does not contain the PASS marker"

grep -Fxq "PIPELINE_VERSION=0x00024E11" "${PACKAGE_STATUS}" ||
    fail 9 "A10 v2 XO package status has the wrong pipeline version"

grep -Fxq "KERNEL_XML_ARG_COUNT=74" "${PACKAGE_STATUS}" ||
    fail 9 "A10 v2 XO package status has the wrong kernel.xml argument count"

grep -Fxq "COMPONENT_XML_REGISTER_COUNT=77" "${PACKAGE_STATUS}" ||
    fail 9 "A10 v2 XO package status has the wrong component.xml register count"
...
grep -qF 'dlrm_f37x_rtl_kernel_stage2n_a10_v2.sv' <<< "${XO_CONTENTS}" ||
    fail 16 "XO does not contain the A10 v2 kernel top"

grep -qF 'dlrm_internal_pipeline_controller.sv' <<< "${XO_CONTENTS}" ||
    fail 17 "XO does not contain the internal pipeline controller"

grep -qF 'dlrm_internal_pipeline_axi_lite_adapter_stage2n_a10_v2.sv'     <<< "${XO_CONTENTS}" ||
    fail 17 "XO does not contain the A10 v2 capacity-expanded adapter"
...
echo "============================================================"
echo "Stage 2N-A10 v1 F37X hardware link"
...
if [[ -n "${TIMING_REPORT}" && -f "${TIMING_REPORT}" ]]; then
    ...
    fail 24 "routed timing summary report was not found"
fi

grep -qF "All user specified timing constraints are met."     "${TIMING_COPY}" ||
    fail 25 "routed timing report does not confirm that all constraints are met"
...
if [[ -z "${TIMING_SUMMARY_LINE}" ]]; then
    TIMING_SUMMARY_LINE="NOT_PARSED"
fi
...
TIMING_SUMMARY_LINE=${TIMING_SUMMARY_LINE}
...
echo "============================================================"
echo "STAGE2N_A10_F37X_LINK_V1_PASS"
echo "XCLBIN=${XCLBIN_PATH}"
...
echo "TIMING_MET=${TIMING_MET}"
echo "TIMING_SUMMARY_LINE=${TIMING_SUMMARY_LINE}"
