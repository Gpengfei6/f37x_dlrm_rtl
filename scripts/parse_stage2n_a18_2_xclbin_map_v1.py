#!/usr/bin/env python3
"""Resolve A18.2 arg0..3 memory indices from xclbin CONNECTIVITY/MEM_TOPOLOGY.

Memory indices are taken from metadata tags, not assumed to be 0..3.
This script never opens a device. Lookup indexes are AXI-Lite MMIO, not args.
"""
...
def parse_host_mem_map_text(text, min_key_size=8):
    ...
    if not lines or lines[0].strip() != MAP_HEADER:
        raise ValueError("memory map header is not A18_2_MEM_MAP_V1")
    mem_index = [None] * 4
    tag = [None] * 4
    uuid_text = None
    for raw in lines[1:]:
...
if __name__ == "__main__":
    try:
        ...
        print("A18_2_MEM_MAP=FAIL", file=sys.stderr)
