#!/usr/bin/env bash
# Protected A18.3 five-tuple flow on the accepted A18.2 xclbin.
# Default action is prepare: source/golden/map checks. Never programs.
# execute requires explicit authorization. Host does not call xbutil.
# If CURRENT_UUID already matches A18, program is skipped.
...
A18_2_HOST_SOURCE="${REPO_ROOT}/host/stage2n_a18_2_four_bo_host_v1.cpp"
...
RESULT_DIR="${REPO_ROOT}/results/stage2n_a18_3/protected_v1/${STAMP}"
mkdir -p "${RESULT_DIR}"
...
require_goldens()
{
    python3 - "${HOST_SOURCE}" "${BASELINE}" <<'PY' || fail "golden lock failed"
import pathlib, sys
host = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8")
baseline = pathlib.Path(sys.argv[2])
assert baseline.stat().st_size == 1024
for token in ("-393, -61, -60, -162, -185",
              "{{37u, 38u, 39u, 40u}}",
              "{{1u, 2u, 3u, 4u}}",
              "{{0u, 0u, 0u, 0u}}",
              "{{0u, 63u, 37u, 40u}}",
              "{{63u, 62u, 61u, 60u}}",
              "load_baseline_all",
              "XSim result 36 is not an A18.3 golden"):
    assert token in host, token
assert "load_restored_case" not in host
print("A18_3_GOLDEN_LOCK=PASS")
PY
}
...
echo "A18_3_PROTECTED_FLOW=START"
echo "ACTION=${ACTION}"
...
echo "LOCKED_TUPLES=37,38,39,40 -> -393; 1,2,3,4 -> -61; 0,0,0,0 -> -60; 0,63,37,40 -> -162; 63,62,61,60 -> -185"
echo "FPGA_PROGRAMMING=NOT_RUN"
echo "BOARD=NOT_RUN"
echo "PERFORMANCE=NOT_CLAIMED"

[[ -s "${HOST_SOURCE}" ]] || fail "A18.3 Host source missing"
[[ -s "${A18_2_HOST_SOURCE}" ]] || fail "A18.2 Host source missing; do not replace it"
...
if [[ "${ACTION}" == "prepare" ]]; then
    ...
    echo "A18_3_HOST_PREPARE=PASS"
    echo "A18_3_BOARD_EXECUTION=NOT_AUTHORIZED"
    echo "FPGA_PROGRAMMING=NOT_RUN"
    echo "HOST_EXECUTION=NOT_RUN"
    echo "BOARD=NOT_RUN"
    echo "PHYSICAL_HBM=NOT_RUN"
    echo "PERFORMANCE=NOT_CLAIMED"
    echo "FIRST_BOARD_RUN_REQUIRES=A18_3_BOARD_EXECUTION_AUTHORIZED=yes A18_3_ALLOW_PROGRAM=yes A18_3_CONFIRM=yes plus execute"
    exit 0
fi
...
[[ "${A18_3_BOARD_EXECUTION_AUTHORIZED:-no}" == "yes" ]] ||
    fail "board execution is not authorized; keep ACTION=prepare"
[[ "${A18_3_ALLOW_PROGRAM:-no}" == "yes" ]] ||
    fail "execute refused: A18_3_ALLOW_PROGRAM is not yes"
...
echo "EXPECTED_XRT_VERSION=${EXPECTED_XRT_VERSION}"
...
echo "============================================================"
echo "Stage 2N-A18.3 protected board authorization"
...
echo "LOCKED_INDEX_TUPLES=37,38,39,40 / 1,2,3,4 / 0,0,0,0 / 0,63,37,40 / 63,62,61,60"
echo "PER_BO_LAYOUT=baseline_table_on_all_four_banks"
echo "A18_2_FIVE_CASE_HOST=NOT_USED"
if [[ "${CURRENT_UUID}" == "${EXPECTED_UUID}" ]]; then
    ...
    echo "NOTE=this replaces the currently loaded xclbin with the reviewed A18 image only"
fi
...
if [[ -z "${confirmation}" ]]; then
    ...
    read -r -p "Proceed with this protected A18.3 five-tuple run? [yes/no] " confirmation
fi
if [[ "${confirmation}" != "yes" ]]; then
    echo "A18.3 protected run cancelled safely."
    echo "FPGA_PROGRAMMING=NOT_RUN"
    echo "BOARD=NOT_RUN"
    exit 0
fi
...
if [[ "${CURRENT_UUID}" != "${EXPECTED_UUID}" ]]; then
    if [[ "${A18_3_FORCE_NO_PROGRAM:-}" == "yes" ]]; then
        fail "UUID is ${CURRENT_UUID}, expected ${EXPECTED_UUID}; A18_3_FORCE_NO_PROGRAM=yes refuses xbutil program"
    fi
    ...
    echo "A18_3_PROGRAM=SKIPPED_ALREADY_LOADED"
fi
...
grep -Fxq "A18_3_BO_CLEANUP=PASS" "${HOST_LOG}" || fail "BO cleanup PASS missing"
...
grep -Fq "A18_2_FIVE_CASE_SENSITIVITY_NOT_USED=1" "${HOST_LOG}" || fail "sensitivity-not-used guard missing"
grep -Fq "PIPE_VERSION_HEX=0x00024e18" "${HOST_LOG}" || fail "PIPE_VERSION missing"
...
grep -Fxq "TUPLE0_LOOKUP_INDEX0=37" "${HOST_LOG}" || fail "TUPLE0 index0 missing"
grep -Fxq "TUPLE1_LOOKUP_INDEX0=1" "${HOST_LOG}" || fail "TUPLE1 index0 missing"
grep -Fxq "TUPLE2_LOOKUP_INDEX0=0" "${HOST_LOG}" || fail "TUPLE2 index0 missing"
grep -Fxq "TUPLE3_LOOKUP_INDEX0=0" "${HOST_LOG}" || fail "TUPLE3 index0 missing"
grep -Fxq "TUPLE3_LOOKUP_INDEX1=63" "${HOST_LOG}" || fail "TUPLE3 index1 missing"
grep -Fxq "TUPLE4_LOOKUP_INDEX0=63" "${HOST_LOG}" || fail "TUPLE4 index0 missing"
for n in 0 1 2 3 4; do
    grep -Fxq "TUPLE${n}_COMPLETE_DLRM_RESULT=PASS" "${HOST_LOG}" || fail "TUPLE${n} complete-DLRM PASS missing"
    grep -Fxq "TUPLE${n}_EMBEDDING_LOADED_MASK=0xF" "${HOST_LOG}" || fail "TUPLE${n} mask missing"
...
echo "A18_3_FPGA_PROGRAMMING=${PROGRAMMING_STATUS}"
echo "A18_3_BOARD_FUNCTIONAL=HOST_RETURNED_PASS"
echo "A18_3_PERFORMANCE=NOT_CLAIMED"
echo "FPGA_RESET=NOT_RUN"
echo "OTHER_DEVICE_ACCESS=NONE"
echo "NOTE=physical five-tuple PASS still requires a separate evidence review"
