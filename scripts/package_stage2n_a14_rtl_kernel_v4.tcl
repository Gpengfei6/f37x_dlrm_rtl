...
# with m_axi_gmem.
#
# Vivado 2020.2 emits kernel.xml port range 0xFFFFFFFF even when the RTL,
# packaged AXI bus/model parameters, IP-XACT address space, and TABLE_BASE
# pointer are all 64-bit. This version records that field but proves address
# width from the stronger component/RTL evidence in the target runner.
#
# This script packages XO metadata only. It does not invoke v++, bind HBM,
# generate an xclbin, access a device, or overwrite earlier A14 build roots.

set script_path [file normalize [info script]]
set script_dir [file dirname $script_path]
set root_dir [file normalize [file join $script_dir ..]]

set build_dir [file join $root_dir build stage2n_a14 xo_v3]
set project_dir [file join $build_dir vivado_project]
set ip_dir [file join $build_dir dlrm_f37x_stage2n_a14_ip_v3]
set top_name "dlrm_f37x_rtl_kernel_stage2n_a14_v2"
set target_part_name "xcvu37p-fsvh2892-2L-e"
set part_name $target_part_name
if {$argc > 1} {
    error "Usage: vivado -source package_stage2n_a14_rtl_kernel_v4.tcl ?packaging_part?"
}
if {$argc == 1} {
...
puts "============================================================"
puts "Stage 2N-A14.5 F37X runtime-table-base XO packaging v4"
puts "ROOT=$root_dir"
...
create_project dlrm_f37x_stage2n_a14_package_v4 $project_dir \
...
set_property description \
    ...
    $core
set_property version 2.16 $core
...
set kernel_port_range ""
if {![regexp {<port[^>]*name="m_axi_gmem"[^>]*range="([^"]+)"} \
      $xml_text ignored kernel_port_range]} {
    fail "kernel.xml m_axi_gmem range field is missing"
}
if {![string equal -nocase $kernel_port_range "0xFFFFFFFF"] &&
    ![string equal -nocase $kernel_port_range "0xFFFFFFFFFFFFFFFF"]} {
    fail "Unexpected kernel.xml m_axi_gmem range: $kernel_port_range"
}

puts "============================================================"
puts "STAGE2N_A14_5_TABLE_BASE_XO_PACKAGE_V2=PASS"
puts "KERNEL=$top_name"
puts "TARGET_PART=$target_part_name"
puts "PACKAGING_PART=$part_name"
puts "TARGET_PART_USED=$target_part_used"
...
puts "TABLE_BASE_ASSOCIATED_BUSIF=m_axi_gmem"
puts "KERNEL_XML_PORT_RANGE=$kernel_port_range"
puts "AXI_ADDRESS_WIDTH_VALIDATION=DEFER_TO_COMPONENT_AND_RTL_GATE"
