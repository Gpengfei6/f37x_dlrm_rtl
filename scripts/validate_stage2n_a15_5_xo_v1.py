#!/usr/bin/env python3
"""Fail-closed Stage 2N-A15.5 XO/source metadata validator.

The accepted A15.4 kernel uses a user-managed AXI4-Lite register space.  Only
TABLE_BASE is an XRT kernel argument; the complete A13/A15 ABI remains visible
as raw register offsets in the accepted wrapper RTL.
"""
...
def validate_component_xml(path):
    ...
    require(parse_scaled_integer(child_text(block, "range", "control range")) >= 0x30C, "control map range too small")
...
def run_self_test():
    ...
    with tempfile.TemporaryDirectory(prefix="a15_5_xo_validator_") as temp_name:
        temp = Path(temp_name)
        paths = {
    ...
    print("A15_5_XO_VALIDATOR_SELF_TEST=PASS")
...
def main():
    ...
    try:
        ...
        raise SystemExit("A15.5 XO validation failed: {}".format(error))
    print("A15_5_TARGET_XO_METADATA_VALIDATION=PASS")
