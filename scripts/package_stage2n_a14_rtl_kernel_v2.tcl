# Stage 2N-A14.4-B: package the standalone A14 HBM lookup wrapper as a
# user-managed Vitis RTL kernel with explicit AXI master metadata.
#
# Vivado 2020.2 defaulted the inferred m_axi_gmem DATA_WIDTH and ADDR_WIDTH
# metadata to 32 even though the RTL ports are 128-bit data and 64-bit address.
# This revision writes both bus parameters explicitly before package_xo.
# It does not modify RTL, invoke v++, link an xclbin, bind physical HBM, access
# a device, program an FPGA, or overwrite an existing build directory.
...
if {$argc > 1} {
    error "Usage: vivado -source package_stage2n_a14_rtl_kernel_v2.tcl ?packaging_part?"
}
if {$argc == 1} {
...
puts "============================================================"
puts "Stage 2N-A14.4-B F37X RTL-kernel XO packaging"
puts "ROOT=$root_dir"
...
puts "M_AXI_METADATA_EXPLICIT=1"
...
create_project dlrm_f37x_stage2n_a14_package_v2 $project_dir \
...
# Vivado 2020.2 otherwise records both values as the 32-bit defaults in the
# generated kernel.xml. These values match the frozen A14 wrapper ports.
...
if {[string first {dataWidth="128"} $xml_text] < 0} {
    fail "kernel.xml does not record the 128-bit AXI master data width"
}
...
puts "============================================================"
puts "STAGE2N_A14_4_B_XO_PACKAGE=PASS"
puts "KERNEL=$top_name"
puts "TARGET_PART=$target_part_name"
puts "PACKAGING_PART=$part_name"
puts "TARGET_PART_USED=$target_part_used"
