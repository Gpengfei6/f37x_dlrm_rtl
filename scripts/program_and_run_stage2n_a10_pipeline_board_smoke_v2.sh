#!/usr/bin/env bash
#
# Stage 2N-A10 v2 protected F37X board smoke for the capacity-expanded five-layer pipeline.
#
# Validates the Stage 2M-shaped deterministic five-layer model after
# the A10 capacity expansion and 100 MHz F37X hardware link.
#
...
EXPECTED_NEW_UUID="f7a23117-5218-4fba-adb5-f093b596df03"
EXPECTED_NEW_KERNEL="dlrm_f37x_rtl_kernel_stage2n_a10_v2"
EXPECTED_NEW_INSTANCE="dlrm_a10_1"
EXPECTED_NEW_CU="${EXPECTED_NEW_KERNEL}:${EXPECTED_NEW_INSTANCE}"
EXPECTED_NEW_SIZE="43078792"

ALLOWED_SOURCE_A7_UUID="5d4ac982-14e3-40fb-afaa-f04ba82dce61"
ALLOWED_SOURCE_A7_CU="dlrm_f37x_rtl_kernel_stage2n_a7:dlrm_a7_1"

XCLBIN="build/stage2n_a10/link_v1/hw/dlrm_f37x_rtl_kernel_stage2n_a10_v2.xclbin"
XCLBIN_INFO="results/stage2n_a10/link_v1/dlrm_f37x_rtl_kernel_stage2n_a10_v2.xclbin.info"
A10_STATUS="results/stage2n_a10/link_v1/stage2n_a10_f37x_link_v1_status.txt"
A10_ACCEPT_STATUS="results/stage2n_a10/link_v1/stage2n_a10_f37x_link_artifact_accept_v1_status.txt"

SOURCE="host/stage2n_a10_pipeline_board_smoke_v2.cpp"
BUILD_DIR="build/stage2n_a10_board_v2"
RESULT_DIR="results/stage2n_a10_board_v2"
SERVER_RESULT_DIR="server_results/stage2n_a10"

mkdir -p "${BUILD_DIR}" "${RESULT_DIR}" "${SERVER_RESULT_DIR}"

STAMP="$(date +%Y%m%d_%H%M%S)"
BINARY="${BUILD_DIR}/stage2n_a10_pipeline_board_smoke_v2_${STAMP}"
LOG="${SERVER_RESULT_DIR}/stage2n_a10_pipeline_board_smoke_v2_${STAMP}.log"
STATUS="${RESULT_DIR}/stage2n_a10_pipeline_board_smoke_v2_status_${STAMP}.txt"
...
write_failure()
{
    ...
    {
        echo "STAGE2N_A10_PIPELINE_BOARD_SMOKE_V2_FAILED"
        echo "TIME=$(date -Is)"
...
echo "============================================================"
echo "Stage 2N-A10 v2 Protected F37X Five-Layer Pipeline Board Smoke"
echo "TIME=$(date -Is)"
echo "REPO=$(pwd)"
...
[[ -s "${SOURCE}" ]] ||
    fail "host source not found or empty: ${SOURCE}"

[[ -s "${XCLBIN}" ]] ||
    fail "A10 xclbin not found or empty: ${XCLBIN}"

[[ -s "${XCLBIN_INFO}" ]] ||
    fail "A10 xclbin info not found: ${XCLBIN_INFO}"

[[ -s "${A10_STATUS}" ]] ||
    fail "A10 link status not found: ${A10_STATUS}"

[[ -s "${A10_ACCEPT_STATUS}" ]] ||
    fail "A10 final acceptance status not found: ${A10_ACCEPT_STATUS}"

grep -q '^STAGE2N_A10_F37X_LINK_V1_PASS$' "${A10_STATUS}" ||
    fail "A10 link PASS marker is missing"

grep -q '^STAGE2N_A10_F37X_LINK_ARTIFACT_ACCEPT_V1_PASS$' \
    "${A10_ACCEPT_STATUS}" ||
    fail "A10 final acceptance PASS marker is missing"
...
echo
echo "========== BUILD A10 HOST BEFORE PROGRAMMING =========="
...
[[ "${BUILD_RESULT}" -eq 0 ]] ||
    fail "A10 host compilation returned ${BUILD_RESULT}"
...
if [[ "${CURRENT_UUID}" == "${EXPECTED_NEW_UUID}" ]]; then
    echo "The Stage 2N-A10 image is already loaded; programming is skipped."

    echo "${PRE_QUERY}" | grep -Fq "${EXPECTED_NEW_CU}" ||
        fail "new UUID is loaded but the expected CU is missing"

    echo "${PRE_QUERY}" | grep -Fq "(IDLE)" ||
        fail "new A10 CU is not IDLE before smoke"
else
    SOURCE_KIND=""
    SOURCE_CU=""

    if [[ "${CURRENT_UUID}" == "${ALLOWED_SOURCE_A7_UUID}" ]]; then
        SOURCE_KIND="verified Stage 2N-A9 A7 image"
        SOURCE_CU="${ALLOWED_SOURCE_A7_CU}"
    else
        fail "current UUID ${CURRENT_UUID} is not the verified A7 source; refusing to program"
    fi
    ...
    read -r -p \
        "Type exactly PROGRAM_STAGE2N_A10_INDEX2_9B to continue: " \
        CONFIRM_1

    [[ "${CONFIRM_1}" == "PROGRAM_STAGE2N_A10_INDEX2_9B" ]] ||
...
echo "${POST_PROGRAM_QUERY}" | grep -Fq "${EXPECTED_NEW_CU}" ||
    fail "new A10 CU is not visible after programming"

echo "${POST_PROGRAM_QUERY}" | grep -Fq "(IDLE)" ||
    fail "new A10 CU is not IDLE after programming"

require_empty_render "POST_PROGRAM"

echo
echo "========== RUN A10 FIVE-LAYER AUTOMATIC PIPELINE BOARD SMOKE =========="
...
[[ "${HOST_RESULT}" -eq 0 ]] ||
    fail "A10 host returned ${HOST_RESULT}"

grep -qF \
    "STAGE2N_A10_PIPELINE_BOARD_SMOKE_V2_PASS runs=2 final=36 bottom=8 interaction=18 descriptors=5 weights=1360 biases=73" \
    "${LOG}" ||
    fail "A10 host PASS marker is missing"
...
echo "${POST_RUN_QUERY}" | grep -Fq "${EXPECTED_NEW_CU}" ||
    fail "A10 CU disappeared after board smoke"

echo "${POST_RUN_QUERY}" | grep -Fq "(IDLE)" ||
    fail "A10 CU is not IDLE after board smoke"

{
    echo "STAGE2N_A10_PIPELINE_BOARD_SMOKE_V2_PASS"
    echo "TIME=$(date -Is)"
    echo "TARGET_INDEX=${TARGET_INDEX}"
    echo "TARGET_BDF=${TARGET_BDF}"
    echo "TARGET_RENDER=${TARGET_RENDER}"
    ...
    echo "PIPELINE_DESCRIPTOR_COUNT=5"
    echo "PIPELINE_WEIGHT_VALUES=1360"
    echo "PIPELINE_BIAS_VALUES=73"
    echo "PIPELINE_MODEL_SHAPE=8x16x8_interact18_32x16x1"
    echo "PIPELINE_EXPECTED_RESULT_TAG=4"
    echo "PIPELINE_RESULT=36"
