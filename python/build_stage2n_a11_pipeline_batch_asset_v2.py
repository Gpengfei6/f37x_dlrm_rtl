...
Outputs:
    build/stage2n_a11/assets_v1/stage2n_a11_real_model_batch_v2.f37xpb
    results/stage2n_a11/assets_v1/stage2n_a11_real_model_batch_v2_manifest.json
    results/stage2n_a11/assets_v1/stage2n_a11_real_model_batch_v2_samples.csv
    results/stage2n_a11/assets_v1/stage2n_a11_real_model_batch_v2_status.txt
...
OUTPUT_MAGIC = b"F37XPB1\x00"
OUTPUT_VERSION = 1
OUTPUT_HEADER_BYTES = 128
OUTPUT_HEADER_FIXED = struct.Struct("<8s15I5Q")

if len(OUTPUT_MAGIC) != 8:
    raise RuntimeError(
        f"F37XPB output magic must be exactly 8 bytes, got {len(OUTPUT_MAGIC)}"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"STAGE2N_A11_PIPELINE_BATCH_ASSET_V2_FAILED: {exc}",
            file=sys.stderr,
        )
        raise SystemExit(1)
