...
// Independent A17 Host. Do not treat this as A16 with four copies of mem_index 0.
// Memory indices come from a metadata-derived map; each BO is filled, synced,
// addressed, and released independently.
...
constexpr std::uint32_t kExpectedPipeVersion = 0x00024E17;
...
MemMap load_mem_map(const std::string& path, const std::string& expected_uuid)
{
    ...
    std::string line;
    if (!std::getline(input, line) || trim(line) != "A17_6_MEM_MAP_V1")
        throw std::runtime_error("memory map header is not A17_6_MEM_MAP_V1");
    MemMap map;
    ...
    while (std::getline(input, line)) {
        ...
        if (key == "UUID") {
            ...
            if (value != "dlrm_f37x_rtl_kernel_stage2n_a17_v1")
                throw std::runtime_error("memory map kernel is not A17");
        } else if (key == "CU") {
            if (value != "dlrm_a17_1")
                throw std::runtime_error("memory map CU is not dlrm_a17_1");
        } else if (key == "IP_NAME") {
            if (value != kIpName)
                throw std::runtime_error("memory map IP name is not A17");
...
ModelAsset load_model(const std::string& path)
{
    ...
    if (reader.offset() != reader.size())
        throw std::runtime_error("A17 model asset has trailing bytes");
    return model;
}
        ...
        try {
            ...
            if (index < 0)
                throw std::runtime_error(
                    "xclIPName2Index failed for A17 CU; A16 identity is rejected");
            ip_index_ = static_cast<unsigned>(index);
            if (ip_index_ != kExpectedIpIndex)
                throw std::runtime_error("unexpected A17 IP index");
...
void require_a13_regression(const CaseResult& result)
{
    ...
    // values. A17 lookup/e2e/residual are measured and are not required to
    ...
    const char* case_name, std::int32_t expected)
{
    ...
    if ((a15_during_result & A15_RESULT_VALID) == 0u)
        throw std::runtime_error("A17 status does not expose result-valid");
    ...
    if (result.hbm_lookup_cycles == 0u ||
        ...
        throw std::runtime_error("A17 latency counter is zero");
    ...
    const std::uint32_t after_clear = poll_register(hal, A_A15_CONTROL,
        [](std::uint32_t value) { return (value & A15_START_READY) != 0; },
        "A17 idle after clear");
    if ((after_clear & (A15_DONE | A15_RESULT_VALID | A15_ERROR)) != 0u)
        throw std::runtime_error("stale A17 status remains after CLEAR");
    return result;
}
...
int main(int argc, char** argv)
{
    try {
        ...
        std::cout << "A17_6_HOST_MEM_MAP=PASS\n";
        for (unsigned arg = 0; arg < kBankCount; ++arg) {
        ...
        if (hal.read(A_PIPE_VERSION) != kExpectedPipeVersion)
            throw std::runtime_error("A17 pipeline version mismatch; expected 0x00024E17");
        configure_model(hal, model);
        FourBo bos(hal.handle(), mem_map);

        std::cout << "A17_6_HOST_START=1\n"
        ...
        for (std::size_t index = 0; index < kCaseCount; ++index) {
            ...
            const CaseResult result = run_case(
                ...
                kCaseNames[index], expected_results[index]);
        ...
        const CaseResult repeat = run_case(
            ...
            kCaseNames[0], expected_results[0]);
        print_case("REPEAT_BASELINE_", kCaseNames[0],
            expected_results[0], repeat, bos);

        bos.release_checked();
        std::cout << "A17_6_BO_CLEANUP=PASS\n"
                  << "A17_6_ALL_FIVE_CASES=PASS\n"
                  << "A17_6_REPEAT_BASELINE=PASS\n"
                  << "A17_6_PHYSICAL_HBM_FUNCTIONAL_HOST=EXECUTED\n"
                  << "A17_6_PERFORMANCE=NOT_CLAIMED\n"
                  << "STAGE2N_A17_6_FOUR_BO_HOST_V1=PASS\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "STAGE2N_A17_6_FOUR_BO_HOST_V1_FAIL\n"
                  << "REASON=" << error.what() << "\n"
                  << "A17_6_BO_CLEANUP=ATTEMPTED_RAII\n";
        return 1;
    }
}
