...
constexpr std::uint32_t EXPECTED_PIPE_VERSION = 0x00024E13;
...
void parse_uuid(const std::string& text, xuid_t uuid)
{
    ...
    if (compact.size() != 32u) {
        throw std::runtime_error(
            "xclbin UUID must contain exactly 32 hexadecimal digits");
    }
...
double cycles_to_microseconds(
    std::uint32_t cycles,
    double kernel_clock_mhz)
{
    if (kernel_clock_mhz <= 0.0) {
        throw std::runtime_error("kernel clock MHz must be positive");
    }
    return static_cast<double>(cycles) / kernel_clock_mhz;
}
...
{
public:
    explicit Hal(const xuid_t expected_uuid)
    {
        std::memcpy(uuid_, expected_uuid, sizeof(uuid_));
...
struct RunResult
{
    ...
    std::uint32_t bottom_cycles;
    std::uint32_t interaction_cycles;
    std::uint32_t top_cycles;
    ...
    const Sample& sample)
{
    ...
    if (result.bottom_cycles == 0u ||
        result.interaction_cycles == 0u ||
        result.top_cycles == 0u ||
        result.total_cycles == 0u) {
        throw std::runtime_error("one or more hardware cycle counters are zero");
    }
    const std::uint64_t stage_cycle_sum =
        static_cast<std::uint64_t>(result.bottom_cycles) +
        static_cast<std::uint64_t>(result.interaction_cycles) +
        static_cast<std::uint64_t>(result.top_cycles);
    if (static_cast<std::uint64_t>(result.total_cycles) <
        stage_cycle_sum) {
        throw std::runtime_error(
            "total hardware cycles do not cover ordered stage cycles");
    }
    ...
    if (hal.read(A_BOTTOM_CYCLES) != result.bottom_cycles ||
        hal.read(A_INTERACTION_CYCLES) != result.interaction_cycles ||
        hal.read(A_TOP_CYCLES) != result.top_cycles ||
        hal.read(A_TOTAL_CYCLES) != result.total_cycles) {
        throw std::runtime_error(
            "hardware cycle counters changed after result retirement");
    }
    ...
    if (hal.read(A_BOTTOM_CYCLES) != result.bottom_cycles ||
        ...
        hal.read(A_TOTAL_CYCLES) != result.total_cycles) {
        throw std::runtime_error(
            "hardware cycle counters changed after clear-done");
    }
...
void write_csv_header(std::ofstream& output)
{
    output
        ...
        << "bottom_cycles,interaction_cycles,top_cycles,total_cycles,"
        << "bottom_hw_us,interaction_hw_us,top_hw_us,total_hw_us\n";
}
    ...
    try {
        if (argc != 5) {
            std::cerr
                ...
                << " <output.csv> <kernel_clock_mhz> <xclbin_uuid>\n";
            return 2;
        }

        const std::string asset_path = argv[1];
        const std::string output_csv_path = argv[2];
        const double kernel_clock_mhz = std::stod(argv[3]);
        if (kernel_clock_mhz <= 0.0) {
            throw std::runtime_error("kernel clock MHz must be positive");
        }
        xuid_t expected_uuid = {};
        parse_uuid(argv[4], expected_uuid);

        std::cout
            << "Stage 2N-A13 cycle-counter Host v1\n"
            << "Access=xclOpenContext+xclRegRead+xclRegWrite\n"
            << "ASSET_PATH=" << asset_path << "\n"
            << "OUTPUT_CSV=" << output_csv_path << "\n"
            << "KERNEL_CLOCK_MHZ=" << kernel_clock_mhz << "\n"
            << "EXPECTED_XCLBIN_UUID="
            << uuid_string(expected_uuid) << "\n"
        ...
        unsigned cycle_counter_valid = 0;
        ...
        for (std::size_t index = 0;
            ...
            output
                ...
                << result.bottom_count << ","
                << result.interaction_count << ","
                << result.bottom_cycles << ","
                << result.interaction_cycles << ","
                << result.top_cycles << ","
                << result.total_cycles << ","
                << cycles_to_microseconds(
                       result.bottom_cycles, kernel_clock_mhz) << ","
                << cycles_to_microseconds(
                       result.interaction_cycles, kernel_clock_mhz) << ","
                << cycles_to_microseconds(
                       result.top_cycles, kernel_clock_mhz) << ","
                << cycles_to_microseconds(
                       result.total_cycles, kernel_clock_mhz) << "\n";
        ...
        if (logit_exact != EXPECTED_SAMPLE_COUNT ||
            ...
            cycle_counter_valid != EXPECTED_SAMPLE_COUNT) {
            throw std::runtime_error(
        ...
        std::cout
            ...
            << "BOTTOM_CYCLES=" << last_result.bottom_cycles << "\n"
            << "INTERACTION_CYCLES="
            << last_result.interaction_cycles << "\n"
            << "TOP_CYCLES=" << last_result.top_cycles << "\n"
            << "TOTAL_CYCLES=" << last_result.total_cycles << "\n"
            << "BOTTOM_HW_US="
            << cycles_to_microseconds(
                   last_result.bottom_cycles, kernel_clock_mhz) << "\n"
            << "INTERACTION_HW_US="
            << cycles_to_microseconds(
                   last_result.interaction_cycles,
                   kernel_clock_mhz) << "\n"
            << "TOP_HW_US="
            << cycles_to_microseconds(
                   last_result.top_cycles, kernel_clock_mhz) << "\n"
            << "TOTAL_HW_US="
            << cycles_to_microseconds(
                   last_result.total_cycles, kernel_clock_mhz) << "\n"
        ...
        std::cerr
            << "STAGE2N_A13_CYCLE_COUNTER_BOARD_V1_FAILED: "
            << error.what() << "\n";
        return 1;
    }
}
