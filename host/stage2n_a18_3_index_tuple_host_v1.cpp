...
// Independent A18.3 Host. Do not edit the boarded A18.2 five-case Host.
// All four BOs always receive the baseline 1024-byte table. Lookup indexes
// are AXI-Lite MMIO from a locked tuple list. Software goldens are not a
// board PASS until a later authorized execute is reviewed.
...
constexpr std::size_t kTupleCount = 5;
constexpr std::uint32_t kRowCount = 64;
constexpr std::array<const char*, kTupleCount> kTupleNames = {{
    "default_37_38_39_40",
    "rows_1_2_3_4",
    "rows_0_0_0_0",
    "rows_0_63_37_40",
...
// Locked from models/stage2n_a18_2/index_goldens_v1.json plus CASE0 -393.
// Slot-sensitivity goldens -392/-93/-689/-519 are A18.2-only. XSim 36 is not
// a golden here. Tuples are not board evidence until a later execute review.
constexpr std::array<std::array<std::uint32_t, kBankCount>, kTupleCount>
    kLockedIndexes = {{
    {{37u, 38u, 39u, 40u}},
    {{1u, 2u, 3u, 4u}},
    {{0u, 0u, 0u, 0u}},
...
constexpr std::array<std::int32_t, kTupleCount> kLockedGoldens = {{
    -393, -61, -60, -162, -185
}};
...
void require_locked_indexes(const std::array<std::uint32_t, kBankCount>& indexes)
{
    for (unsigned slot = 0; slot < kBankCount; ++slot) {
        if (indexes[slot] >= kRowCount)
...
public:
    ...
    // A18.3 never mutates a slot image. Sensitivity tables belong to A18.2.
    void load_baseline_all(const std::vector<std::uint8_t>& baseline)
    {
        for (unsigned index = 0; index < kBankCount; ++index) {
            bos_[index]->load_and_sync(baseline);
            image_name_[index] = "baseline";
        }
    }
...
CaseResult run_tuple(Hal& hal, FourBo& bos, const ModelAsset& model,
    const std::vector<std::uint8_t>& baseline, std::int32_t expected,
    const std::array<std::uint32_t, kBankCount>& lookup_indexes)
{
    require_locked_indexes(lookup_indexes);
    write_dense(hal, model.dense);
    bos.load_baseline_all(baseline);
...
int main(int argc, char** argv)
{
    try {
        ...
        const std::vector<std::uint8_t> baseline = read_file(argv[6]);
        if (baseline.size() != kPayloadBytes)
            throw std::runtime_error("baseline table must be exactly 1024 bytes");
        std::cout << "A18_3_HOST_MEM_MAP=PASS\n";
        for (unsigned arg = 0; arg < kBankCount; ++arg) {
        ...
        std::cout << "A18_3_HOST_START=1\n"
                  ...
                  << "A18_2_FIVE_CASE_SENSITIVITY_NOT_USED=1\n"
                  << "FUNCTIONAL_TUPLES=" << kTupleCount << "\n"
        ...
        for (std::size_t index = 0; index < kTupleCount; ++index) {
            if (kLockedGoldens[index] == 36)
                throw std::runtime_error("XSim result 36 is not an A18.3 golden");
            require_locked_indexes(kLockedIndexes[index]);
            const CaseResult result = run_tuple(
                hal, bos, model, baseline, kLockedGoldens[index],
                kLockedIndexes[index]);
            if (have_previous && result.actual == previous_actual &&
                kLockedGoldens[index] != previous_actual)
                throw std::runtime_error("stale result reused across tuples");
            print_case("TUPLE" + std::to_string(index) + "_",
                kTupleNames[index], kLockedGoldens[index], result, bos);
            previous_actual = result.actual;
            have_previous = true;
        }

        bos.release_checked();
        std::cout << "A18_3_BO_CLEANUP=PASS\n"
                  << "A18_3_ALL_FIVE_TUPLES=PASS\n"
                  << "A18_3_PHYSICAL_HBM_FUNCTIONAL_HOST=EXECUTED\n"
                  << "A18_3_PERFORMANCE=NOT_CLAIMED\n"
                  << "STAGE2N_A18_3_INDEX_TUPLE_HOST_V1=PASS\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "STAGE2N_A18_3_INDEX_TUPLE_HOST_V1_FAIL\n"
                  << "REASON=" << error.what() << "\n"
                  << "A18_3_BO_CLEANUP=ATTEMPTED_RAII\n";
        return 1;
    }
}
