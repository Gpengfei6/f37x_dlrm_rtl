...
constexpr const char* kIpName =
    "dlrm_f37x_rtl_kernel_stage2n_a16_v1:dlrm_a16_1";
constexpr unsigned kExpectedIpIndex = 0;
constexpr unsigned kExpectedMemIndex = 0;  // HBM[0] in the future A16.2 xclbin
...
ModelAsset load_model(const std::string& path)
{
    ...
    if (reader.bytes(8) != "F37XA156")
        throw std::runtime_error("A16.2 model magic mismatch");
    ...
    if (version != 1u || header_bytes != 96u || descriptor_count != 5u ||
        ...
        throw std::runtime_error("A16.2 model header contract mismatch");
    ...
    if (reader.offset() != reader.size())
        throw std::runtime_error("A16.2 model asset has trailing bytes");
    return model;
}
    ...
    const std::vector<std::uint8_t>& payload, std::int32_t expected)
{
    ...
    CaseResult result = {
        ...
        hal.read(A_HBM_LOOKUP_CYCLES),
        hal.read(A_FPGA_END_TO_END_CYCLES),
        0,
        base_lo, base_hi
    };
    ...
    if (result.hbm_lookup_cycles == 0u ||
        ...
        throw std::runtime_error("A16 latency counter is zero");
    ...
    try {
        ...
        for (std::size_t index = 0; index < kCaseCount; ++index) {
            ...
            std::cout << prefix << "NAME=" << kCaseNames[index] << "\n"
                      ...
                      << prefix << "COMPUTE_TOTAL_CYCLES=" << result.total_cycles << "\n"
                      << prefix << "HBM_LOOKUP_CYCLES=" << result.hbm_lookup_cycles << "\n"
                      << prefix << "FPGA_END_TO_END_CYCLES=" << result.fpga_end_to_end_cycles << "\n"
                      << prefix << "PIPELINE_OVERHEAD_CYCLES=" << result.pipeline_overhead_cycles << "\n"
                      << prefix << "LATENCY_ACCOUNTING=PASS\n"
                      << prefix << "COMPLETE_DLRM_RESULT=PASS\n";
        }

        bo.release_checked();
        std::cout << "A16_2_BO_CLEANUP=PASS\n"
        ...
        std::cerr << "STAGE2N_A16_2_PHYSICAL_LATENCY_V1_FAIL\n"
                  << "REASON=" << error.what() << "\n"
                  << "A16_2_BO_CLEANUP=ATTEMPTED_RAII\n";
        return 1;
    }
}
