# Stage 2N-A8 v3: package the Stage 2N-A7 automatic DLRM pipeline
...
set build_dir [file join $root_dir build stage2n_a8 package_v3]
set project_dir [file join $build_dir vivado_project]
set ip_dir [file join $build_dir dlrm_f37x_stage2n_a7_ip_v3]
set xo_path [file join $build_dir dlrm_f37x_rtl_kernel_stage2n_a7.xo]
set xml_path [file join $build_dir kernel.xml]

set part_name "xcvu37p-fsvh2892-2L-e"
set top_name "dlrm_f37x_rtl_kernel_stage2n_a7"
...
set source_a7_adapter \
    [find_tracked_unique $root_dir dlrm_internal_pipeline_axi_lite_adapter_stage2n_a7.sv]
set source_a7_top \
    [find_tracked_unique $root_dir dlrm_f37x_rtl_kernel_stage2n_a7.sv]
...
puts "============================================================"
puts "Stage 2N-A8 v3 F37X A7 RTL-kernel XO packaging"
puts "Root : $root_dir"
puts "Part : $part_name"
puts "Top  : $top_name"
puts "XO   : $xo_path"
...
puts "A7_TOP_SOURCE=$source_a7_top"
puts "============================================================"

create_project dlrm_f37x_stage2n_a8_package_v3 $project_dir \
    -part $part_name \
    -force
...
set_property display_name \
    {F37X DLRM Automatic Bottom-Interaction-Top RTL Kernel} \
    $core
set_property description \
    {Stage 2N-A7 user-managed AXI4-Lite kernel preserving the legacy MLP and standalone interaction windows while adding one-command Bottom MLP to feature interaction to Top MLP execution} \
    $core
set_property version 2.7 $core
...
# ---------------------------------------------------------------------------
# 0x180-0x217: Stage 2N-A7 automatic internal-pipeline window.
# ---------------------------------------------------------------------------
...
add_control_register $address_block PIPE_VERSION 0x184 read-only \
    {Stage 2N-A7 automatic-pipeline interface version}
...
puts "============================================================"
puts "STAGE2N_A8_XO_PACKAGE_V3_PASS"
puts "XO=$xo_path"
