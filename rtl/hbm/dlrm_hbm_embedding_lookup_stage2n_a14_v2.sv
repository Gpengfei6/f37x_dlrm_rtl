...
// Stage 2N-A14.5 single-bank lookup with a runtime 64-bit table base.
//
// The accepted A14.1 v1 block uses table-relative addresses.  This version is
// intentionally separate and adds the allocation base needed by a future XRT
// BO-backed HBM table without changing the row format or AXI read protocol.
module dlrm_hbm_embedding_lookup_stage2n_a14_v2 #(
  ...
  input  logic [AXI_ADDR_WIDTH-1:0]     table_base_addr,
  ...
  logic [AXI_ADDR_WIDTH-1:0] request_row_offset;
  logic [AXI_ADDR_WIDTH:0] request_address_sum;

  assign request_row_offset =
      {{(AXI_ADDR_WIDTH-INDEX_WIDTH){1'b0}}, lookup_req_index} << 4;
  assign request_address_sum =
      {1'b0, table_base_addr} + {1'b0, request_row_offset};
      ...
      case (state)
        IDLE: begin
          if (lookup_req_valid && lookup_req_ready) begin
            ...
            if ((lookup_req_index >= ROWS) ||
                (table_base_addr[3:0] != 4'b0000) ||
                request_address_sum[AXI_ADDR_WIDTH]) begin
              response_error_reg <= 1'b1;
              state <= RESP;
            end else begin
              address_reg <= request_address_sum[AXI_ADDR_WIDTH-1:0];
              state <= SEND_AR;
            end
          end
        end
  ...
  initial begin
    if (ROWS <= 0)
      $error("A14.5 HBM lookup ROWS must be positive");
    if (DIM <= 0 || ELEMENT_WIDTH <= 0 || DATA_WIDTH <= 0)
      $error("A14.5 HBM lookup dimensions and widths must be positive");
    if (DIM * ELEMENT_WIDTH != DATA_WIDTH)
      $error("A14.5 HBM lookup DATA_WIDTH must equal DIM*ELEMENT_WIDTH");
    if (DATA_WIDTH != 128 || ROW_BYTES != 16)
      $error("A14.5 HBM lookup requires a 128-bit, 16-byte row");
    if (AXI_ADDR_WIDTH != 64)
      $error("A14.5 HBM lookup requires a 64-bit AXI address");
    if (AXI_ADDR_WIDTH < INDEX_WIDTH + 4)
      $error("A14.5 HBM lookup AXI address width is too small");
  end
endmodule
