...
// Stage 2N-A14.5 standalone Vitis RTL-kernel wrapper.
//
...
//
// TABLE_BASE is captured together with LOOKUP_INDEX on an accepted START.  The
// lookup then issues one read at TABLE_BASE + LOOKUP_INDEX*16.  The accepted
// A13 top and the A14 v1 relative-address prototype remain untouched.
module dlrm_f37x_rtl_kernel_stage2n_a14_v2 #(
    ...
    logic [C_M_AXI_GMEM_ADDR_WIDTH-1:0] table_base_reg;
    logic [C_M_AXI_GMEM_ADDR_WIDTH-1:0] lookup_request_base_reg;
    logic start_pending;
    logic done_latched;
    ...
    // The A14.5 prototype remains read-only. Expose the complete AXI4 port
    // bundle required by Vitis while keeping every write channel inactive.
        ...
        if (!ap_rst_n) begin
            ...
            if (aw_pending && w_pending &&
                ...
                case (awaddr_reg)
                        ...
                        if (wstrb_reg[0] && wdata_reg[0]) begin
                            if (lookup_start_ready) begin
                                lookup_request_index_reg <=
                                    lookup_index_reg;
                                lookup_request_base_reg <= table_base_reg;
                                start_pending <= 1'b1;
                                response_error_latched <= 1'b0;
                            end else begin
                    ...
                    ADDR_TABLE_BASE_LO: begin
                        table_base_reg[31:0] <= apply_wstrb32(
                            table_base_reg[31:0],
                            wdata_reg,
                            wstrb_reg
                        );
                    ...
                    ADDR_TABLE_BASE_HI: begin
                        table_base_reg[63:32] <= apply_wstrb32(
                            table_base_reg[63:32],
                            wdata_reg,
                            wstrb_reg
                        );
                    end
            ...
            if (s_axi_control_arvalid && s_axi_control_arready) begin
                ...
                case (s_axi_control_araddr)
                    ...
                    ADDR_TABLE_BASE_LO:
                        s_axi_control_rdata <= table_base_reg[31:0];
                    ADDR_TABLE_BASE_HI:
                        s_axi_control_rdata <= table_base_reg[63:32];
                    ADDR_RESULT0:
    ...
    dlrm_hbm_embedding_lookup_stage2n_a14_v2 #(
        ...
        .table_base_addr(lookup_request_base_reg),
    ...
    initial begin
        if (C_S_AXI_CONTROL_ADDR_WIDTH < 6)
            $error("A14.5 AXI-Lite address width must cover 0x2C");
        if (C_S_AXI_CONTROL_DATA_WIDTH != 32)
            $error("A14.5 AXI-Lite data width must be 32 bits");
        if (C_M_AXI_GMEM_ADDR_WIDTH != 64)
            $error("A14.5 m_axi_gmem address width must be 64 bits");
        if (C_M_AXI_GMEM_DATA_WIDTH != 128)
            $error("A14.5 m_axi_gmem data width must be 128 bits");
        if (ROWS != 64 || DIM != 8 || ELEMENT_WIDTH != 16)
            $error("A14.5 prototype requires ROWS=64, DIM=8, INT16");
    end
