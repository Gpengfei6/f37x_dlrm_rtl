...
// Stage 2N-A15.4 v1 F37X kernel integration top.
//
...
// its four read-only cycle counters. A15.4 adds only the disjoint 0x300-0x308
// control window and one public m_axi_gmem read master.
module dlrm_f37x_rtl_kernel_stage2n_a15_v1 #(
    ...
    dlrm_internal_pipeline_axi_lite_adapter_stage2n_a15_v1 #(
...
// Stage 2N-A15.4 AXI-Lite adapter for the all-HBM embedding pipeline.
//
// The complete accepted A13 window through 0x224 is preserved. A15.4 adds
// CONTROL_STATUS at 0x300 and a 64-bit TABLE_BASE at 0x304/0x308. The new
// command sequence performs four sequential lookups, waits for mask 0xF, and
// then starts the unchanged Bottom -> Interaction -> Top pipeline.
module dlrm_internal_pipeline_axi_lite_adapter_stage2n_a15_v1 #(
    parameter integer C_S_AXI_CONTROL_ADDR_WIDTH = 12,
    parameter integer C_S_AXI_CONTROL_DATA_WIDTH = 32,
    parameter integer C_M_AXI_GMEM_ADDR_WIDTH = 64,
    parameter integer C_M_AXI_GMEM_DATA_WIDTH = 128,
    ...
    initial begin
        if (C_S_AXI_CONTROL_DATA_WIDTH != 32)
            $error("A15.4 pipeline adapter requires a 32-bit AXI-Lite bus");
        if (NUM_PE != 16)
            $error("A15.4 pipeline adapter requires NUM_PE=16");
        if (INPUT_WIDTH != 16 || OUTPUT_WIDTH != 16)
            $error("A15.4 pipeline adapter requires signed INT16 activations");
        if (C_M_AXI_GMEM_ADDR_WIDTH != 64)
            $error("A15.4 m_axi_gmem address width must be 64 bits");
        if (C_M_AXI_GMEM_DATA_WIDTH != 128)
            $error("A15.4 m_axi_gmem data width must be 128 bits");
    end
