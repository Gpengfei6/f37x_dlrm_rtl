...
// Stage 2N-A13 v1 AXI-Lite adapter with hardware pipeline cycle counters.
//
// Register window: 0x180-0x227. Existing A10 v2 addresses are unchanged;
// 0x218-0x224 add four read-only 32-bit cycle counters.
...
module dlrm_internal_pipeline_axi_lite_adapter_stage2n_a13_v1 #(
    ...
    initial begin
        if (C_S_AXI_CONTROL_DATA_WIDTH != 32)
            $error("A13 v1 pipeline adapter requires a 32-bit AXI-Lite bus");
        if (NUM_PE != 16)
            $error("A13 v1 pipeline adapter requires NUM_PE=16");
        if (INPUT_WIDTH != 16 || OUTPUT_WIDTH != 16)
            $error("A13 v1 pipeline adapter requires signed INT16 activations");
    end
