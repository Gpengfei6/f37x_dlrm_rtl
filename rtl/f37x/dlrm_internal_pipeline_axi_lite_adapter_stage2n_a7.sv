...
// Stage 2N-A7 AXI-Lite adapter for the verified Stage 2N-A5 internal pipeline.
//
...
module dlrm_internal_pipeline_axi_lite_adapter_stage2n_a7 #(
    ...
    parameter integer MAX_WEIGHT_VALUES = 1024,
    parameter integer MAX_BIAS_VALUES = 64,
        ...
        if (!ap_rst_n) begin
            ...
            if (ar_fire) begin
                case (s_axi_control_araddr)
                    ...
                    ADDR_PIPE_VERSION:
                        s_axi_control_rdata <= 32'h0002_4E07;
    ...
    initial begin
        if (C_S_AXI_CONTROL_DATA_WIDTH != 32)
            $error("A7 pipeline adapter requires a 32-bit AXI-Lite bus");
        if (NUM_PE != 16)
            $error("A7 pipeline adapter requires NUM_PE=16");
        if (INPUT_WIDTH != 16 || OUTPUT_WIDTH != 16)
            $error("A7 pipeline adapter requires signed INT16 activations");
    end
