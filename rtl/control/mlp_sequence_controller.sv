...
module mlp_sequence_controller #(
  ...
  assign dense_job_in_dim = desc_in_dim[current_layer];
  assign dense_job_out_dim = desc_out_dim[current_layer];
  assign dense_job_input_buffer_select =
      initial_buffer_reg ^ current_layer[0];
  assign dense_job_output_buffer_select =
      ~(initial_buffer_reg ^ current_layer[0]);
  assign dense_job_weight_offset = desc_weight_base[current_layer];
  assign dense_job_bias_offset = desc_bias_base[current_layer];
  assign dense_job_output_shift = desc_output_shift[current_layer];
  assign dense_job_relu_enable = desc_relu_enable[current_layer];
  assign dense_job_tag = current_layer;

  assign result_valid =
      final_layer_active && dense_result_valid;
  ...
  always_comb begin
    ...
    if ((layer_count < 1) || (layer_count > MAX_LAYERS)) begin
      validation_ok = 1'b0;
      ...
      for (validation_index = 0;
        ...
        if (validation_ok && (validation_index < layer_count)) begin
          if (!desc_loaded[validation_index]) begin
            validation_ok = 1'b0;
            validation_error = ERROR_MISSING_DESCRIPTOR;
          end else if (
              (desc_in_dim[validation_index] < 1) ||
              (desc_out_dim[validation_index] < 1) ||
              (desc_in_dim[validation_index] > MAX_IN_DIM) ||
              (desc_out_dim[validation_index] > MAX_OUT_DIM)) begin
            validation_ok = 1'b0;
            validation_error = ERROR_BAD_DIMENSION;
          end else if (
              desc_output_shift[validation_index] > ACC_WIDTH) begin
            validation_ok = 1'b0;
            validation_error = ERROR_BAD_SHIFT;
          end else if (
              (validation_index > 0) &&
              (desc_out_dim[validation_index-1] !=
               desc_in_dim[validation_index])) begin
            validation_ok = 1'b0;
            validation_error = ERROR_DIMENSION_MISMATCH;
          end else begin
            validation_weight_end =
                desc_weight_base[validation_index] +
                (desc_in_dim[validation_index] *
                 desc_out_dim[validation_index]);
            validation_bias_end =
                desc_bias_base[validation_index] +
                desc_out_dim[validation_index];
  ...
  initial begin
    if (MAX_LAYERS < 1)
      $error("mlp_sequence_controller MAX_LAYERS must be positive");
    if (MAX_LAYERS > (1 << LAYER_INDEX_WIDTH))
      $error("mlp_sequence_controller LAYER_INDEX_WIDTH is too small");
    if (JOB_TAG_WIDTH < LAYER_INDEX_WIDTH)
      $error("mlp_sequence_controller JOB_TAG_WIDTH is too small");
    if (DESCRIPTOR_WIDTH < 93)
      $error("mlp_sequence_controller DESCRIPTOR_WIDTH must be at least 93");
  end
