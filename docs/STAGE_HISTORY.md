...
| Stage 2N-A14.5 | Add a runtime HBM table-base ABI | Added versioned v2 lookup/wrapper with 64-bit `TABLE_BASE`, base-plus-row addressing, invalid-base/overflow guards, independent TBs, and XSim/XO-only validation runners | Commit `29401e5`; `docs/STAGE2N_A14_5_HBM_TABLE_BASE_ABI.md`; A14 v2 RTL/TBs/scripts | **Current stage**: attempt-1 XSim failed on a diagnosed standalone-TB 32/6-bit parameter-binding omission; fix present, retry and exact-VU37P XO v2 NOT RUN; no v++, xclbin, Host, HBM, board, or A13 integration |
