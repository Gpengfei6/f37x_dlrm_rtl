...
| Stage 2N-A14.6 | Link the accepted v2 XO without device access | Freeze an exact-XO, one-CU, `m_axi_gmem -> HBM[0]`, 100 MHz F37X link-only flow and add versioned config/runner/validator | `docs/STAGE2N_A14_6_LINK_ONLY_PLAN.md`; `config/stage2n_a14_6_target_v1.cfg`; A14.6 scripts | **Source preparation PASS; target NOT RUN**. Local syntax and synthetic metadata-validator checks only; no v++, xclbin, timing, Host, device, physical HBM, board, performance, or A13 integration result yet |
