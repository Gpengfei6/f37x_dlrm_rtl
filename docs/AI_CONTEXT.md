...
> Current research update — 2026-09-24: Next step is a written UCNN comparison for exact reuse. No mechanism is claimed. C, B, and D are not started. D-095. No RTL. PERFORMANCE=NOT_CLAIMED.
...
7.    the current-stage documents:
   ...
   `docs/STAGE2N_A18_8_T8_CACHED_LOOKUP_V1.md` (cache in T=8 lookup),
   `docs/STAGE2N_A18_9_FOLDED_LOOKUP_XSIM_V1.md` (4-line cache, fold, 2022.1 XSim),
   `docs/STAGE2N_A18_10_FOLDED_INJECT_V1.md` (fold inject handshake + software A13 goldens),
...
Current stage: **Stage 2N-A18.10 folded-inject (local Vivado 2022.1
XSim PASS, not 2020.2) plus fold-then-A13 software goldens; A18.3
extra-run runner ready / board NOT_RUN; A18.3 five-tuple board PASS
`20260917_202130` on GitHub `bcd3f86`; PERFORMANCE=NOT_CLAIMED**.
...
A18.5–A18.10 are not wired into the boarded A18 kernel.
...
A18.5–A18.10 add local mapper, T=8 lookup, cache, fold, inject handshake,
and 2022.1 XSim. Extra-run runner is ready; board extra-run is NOT_RUN.
...
A15.6 keeps that exact xclbin frozen and adds a low-level XRT 2020.2 Host, a
fail-closed protected runner, deterministic trained-model assets, an evidence
assembler, and an offline validator. One baseline plus four slot-sensitivity
...
### Proven by A15.6 local preparation

- the frozen A15.4 wrapper SHA256 is unchanged and the accepted A15.5 xclbin
  identity is consumed without rebuild;
- one explicit-device, one-BDF, one-BO, `HBM[0]`-only protected runner and
  low-level XRT 2020.2 Host flow are prepared;
- the accepted A13 model programming, result retirement, and cycle-counter ABI
  are preserved while all four Host embedding writes remain unused;
- one baseline and four slot-sensitivity payloads pass deterministic generation
  and static validation, with five pairwise-distinguishing expected results;
...
- the versioned preflight contains only Git/toolchain/build/artifact/golden/
  protection checks and creates non-overwriting status evidence;
- local static validation confirms the frozen ABI and legacy XRT 2020.2 Host
  API/build convention;
