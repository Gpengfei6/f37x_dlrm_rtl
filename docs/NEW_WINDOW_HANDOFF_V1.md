...
| **A18.10** | **fold 注入握手 + fold-then-A13 软件金值；2022.1 XSim PASS** | 完整 T=8 DLRM RTL；板上 |
...
| D-091 | A18.10 fold 注入握手 + fold-then-A13 软件金值 |
...
已授权、本地源码已落地：A18.5 映射器、A18.6 T=8 查找、A18.7 一行 cache、A18.8 带 cache 的 T=8 查找、A18.9 四行 cache + pair-fold（2022.1 XSim PASS）、A18.10 fold 注入握手 + 软件金值、A18.3 extra-run runner。
