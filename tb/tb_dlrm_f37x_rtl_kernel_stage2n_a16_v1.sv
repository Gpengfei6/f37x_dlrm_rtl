...
module tb_dlrm_f37x_rtl_kernel_stage2n_a16_v1;
    ...
    dlrm_f37x_rtl_kernel_stage2n_a16_v1 dut (
        .ap_clk(ap_clk),
        .ap_rst_n(ap_rst_n),
    ...
    // A short-width instance of the exact production counter provides a
    // practical directed saturation test without simulating 2^32 cycles.
    ...
    task automatic service_lookup(
        ...
        begin
            ...
            // The accepted A13 loaded mask is sticky across A15 CLEAR.  The
            // second deterministic run therefore rewrites already-loaded
            // slots without changing the visible 4'hF mask.
            if (run_index == 0) begin
                expected_mask_before = (4'b0001 << slot) - 1'b1;
                expected_mask_after =
                    expected_mask_before | (4'b0001 << slot);
            end else begin
                expected_mask_before = 4'hF;
                expected_mask_after = 4'hF;
            end
    ...
    task automatic verify_saturating_counter;
        begin
            ...
            if ((saturation_count !== 4'hF) || saturation_active)
                $fatal(1,
    ...
    always @(posedge ap_clk) begin
        if (!ap_rst_n)
            ...
            if (cycle_count > 3000000)
                $fatal(1,
                       "tb_dlrm_f37x_rtl_kernel_stage2n_a16_v1: TIMEOUT");
        end
    end
    ...
    initial begin
        ...
        // Reset must clear every A16.1-visible state.
        reset_dut();
        axi_read(ADDR_A15_CONTROL_STATUS, a15_status_value);
        ...
        $display("A16_1_RESET_CLEAN=PASS");
        $display("A16_1_SATURATION=PASS");
        ...
        wait_a15_idle();
        $display("A16_1_SLOT0_ERROR_PROTECTION=PASS");
        ...
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_CLEAR);
        wait_a15_idle();
        $display("A16_1_MID_SEQUENCE_ERROR_PROTECTION=PASS");

        // Negative G: legacy Host embedding writes remain address-compatible
        // but the A16.1 HBM-owned path rejects their commit.
        reset_dut();
        ...
        axi_write(ADDR_PIPE_CONTROL_STATUS, PIPE_CMD_ERROR_ACK);
        $display("A16_1_HOST_EMBEDDING_REJECT=PASS");
        ...
        // Two identical runs prove counter restart determinism without a DUT
        // reset. A15 CLEAR returns the sequencer to IDLE; each accepted START
        // restarts both A16 intervals and the accepted A13 counters.
        for (run_index = 0; run_index < 2; run_index = run_index + 1) begin
            ...
            if ((run_index == 0) && (mask_value[3:0] !== 4'h0))
                $fatal(1, "first-run mask must be zero before A16 START");
            ...
            if (mask_value[3:0] !== 4'hF)
                $fatal(1, "final embedding mask mismatch: %h",
                       mask_value[3:0]);
            ...
            $display("A16_1_RUN_%0d=PASS", run_index + 1);
        end

        $display("A16_1_TABLE_BASE_CAPTURE=PASS");
        $display("A16_1_FOUR_LOOKUPS=PASS");
        $display("A16_1_FOUR_SLOT_INJECTION=PASS");
        $display("A16_1_LOADED_MASK=PASS");
        $display("A16_1_PIPELINE_START_GUARD=PASS");
        $display("A16_1_DELAYED_ARREADY=PASS");
        $display("A16_1_DELAYED_RVALID=PASS");
        $display("A16_1_RESPONSE_RETENTION_REUSED=PASS");
        $display("A16_1_REPEATED_START_REJECT=PASS");
        $display("A16_1_END_TO_END_PIPELINE=PASS");
        $display("A16_1_RESULT_MATCH=PASS");
        $display("A16_1_A13_COUNTER_ABI=PASS");
        $display("A16_1_A13_PUBLIC_ABI=PASS");
        $display("A16_1_COUNTER_RESTART_DETERMINISM=PASS");
        $display("A16_1_COUNTER_FREEZE=PASS");
        $display("A16_1_CANONICAL_ROWS=37,38,39,40");
        $display("A16_1_GOLDEN_FINAL_RESULT=36");
        $display("A16_1_ACTUAL_FINAL_RESULT=%0d", held_result);
        $display("A16_1_HBM_LOOKUP_CYCLES=%0d",
                 held_hbm_lookup_cycles);
        $display("A16_1_BOTTOM_CYCLES=%0d", held_bottom_cycles);
        $display("A16_1_INTERACTION_CYCLES=%0d",
                 held_interaction_cycles);
        $display("A16_1_TOP_CYCLES=%0d", held_top_cycles);
        $display("A16_1_COMPUTE_TOTAL_CYCLES=%0d", held_total_cycles);
        $display("A16_1_FPGA_END_TO_END_CYCLES=%0d",
                 held_fpga_end_to_end_cycles);
        $display("A16_1_PIPELINE_OVERHEAD_CYCLES=%0d",
                 pipeline_overhead_cycles);
        $display("A16_1_LOOKUP_REQUESTS=%0d", lookup_request_count);
        $display("A16_1_AR_HANDSHAKES=%0d", ar_handshake_count);
        $display("A16_1_R_HANDSHAKES=%0d", r_handshake_count);
        $display("A16_1_SLOT_INJECTIONS=%0d", slot_injection_count);
        $display("A16_1_FINAL_LOADED_MASK=0x%0h", mask_value[3:0]);
        $display("A16_1_FUNCTIONAL_REGRESSION=PASS");
        $display("A16_1_COUNTER_SEMANTICS=PASS");
        $display("A16_1_LATENCY_ACCOUNTING=PASS");
        $display("STAGE2N_A16_1_SEQUENTIAL_HBM_LATENCY_XSIM_V1_PASS");
        $finish;
    end
