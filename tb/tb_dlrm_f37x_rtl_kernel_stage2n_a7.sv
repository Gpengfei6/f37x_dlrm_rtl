...
module tb_dlrm_f37x_rtl_kernel_stage2n_a7;
    ...
    dlrm_f37x_rtl_kernel_stage2n_a7 dut (
        .ap_clk(ap_clk),
        .ap_rst_n(ap_rst_n),
    ...
    task automatic write_bottom_input;
        begin
            ...
            axi_write(ADDR_PIPE_ACT_LANE_MASK, 32'h0000_000F);
            axi_write(ADDR_PIPE_ACT_DATA0, pack_pair(1, 2));
            axi_write(ADDR_PIPE_ACT_DATA1, pack_pair(3, 4));
            axi_write(ADDR_PIPE_ACT_DATA2, 0);
            axi_write(ADDR_PIPE_ACT_DATA3, 0);
    ...
    task automatic start_pipeline;
        begin
            // base=0, layers=1, initial buffer=0
            axi_write(ADDR_PIPE_BOTTOM_CONFIG, 32'h0000_0100);
            // base=1, layers=1, top input buffer=0
            axi_write(ADDR_PIPE_TOP_CONFIG, 32'h0000_0101);
            // interaction shift=0
    ...
    always @(posedge ap_clk) begin
        if (!ap_rst_n)
            ...
            if (cycle_count > 500000)
                $fatal(1,
                       "tb_dlrm_f37x_rtl_kernel_stage2n_a7: TIMEOUT");
        end
    end
    ...
    initial begin
        ...
        if (version_value !== 32'h0002_4E07)
            $fatal(1, "pipeline version mismatch: 0x%08x",
                   version_value);

        descriptor_value = pack_descriptor(4, 8, 0, 0, 0, 0);
        write_descriptor(0, descriptor_value);
        descriptor_value = pack_descriptor(18, 1, 32, 8, 0, 0);
        write_descriptor(1, descriptor_value);

        for (address_index = 0; address_index < 32;
        ...
        for (address_index = 32; address_index < 50;
             address_index = address_index + 1)
            write_weight(address_index, 1);

        for (address_index = 0; address_index < 9;
             address_index = address_index + 1)
            write_bias(address_index, 0);

        write_embedding(0, 1, 0, -1, 0, 1, 0, -1, 0);
        write_embedding(1, 2, 2, 2, 2, 2, 2, 2, 2);
        write_embedding(2, -1, -2, -3, -4, -5, -6, -7, -8);
        write_embedding(3, 10, 9, 8, 7, 6, 5, 4, 3);
        ...
        $display("PIPELINE_WINDOW_VERSION=0x00024e07");
        $display("PIPELINE_START_COMMANDS=2");
        $display("PIPELINE_RESULT=-60");
        $display("BOTTOM_OUTPUTS=8");
        $display("INTERACTION_OUTPUTS=18");
        $display("FINAL_BACKPRESSURE_CYCLES=12");
        $display("tb_dlrm_f37x_rtl_kernel_stage2n_a7: PASS runs=2 final=-60 legacy_windows=2");
        $finish;
    end
