...
module tb_dlrm_f37x_rtl_kernel_stage2n_a14_v2;
    ...
    localparam logic [CONTROL_ADDR_WIDTH-1:0]
        ADDR_TABLE_BASE_LO = 12'h018;
    localparam logic [CONTROL_ADDR_WIDTH-1:0]
        ADDR_TABLE_BASE_HI = 12'h01C;
    localparam logic [CONTROL_ADDR_WIDTH-1:0]
    ...
    logic [GMEM_ADDR_WIDTH-1:0] current_expected_base;
    ...
    task automatic wait_for_done(
        ...
        begin
            ...
            while (!done_seen && poll_count < 200) begin
                ...
                if (done_seen && control_word[4] !== expected_error)
                    $fatal(1,
                           "Lookup error mismatch index=%0d expected=%0b actual=%0b",
                           current_expected_index,
                           expected_error,
                           control_word[4]);
                if (!done_seen && control_word[4] !== 1'b0)
                    $fatal(1,
                           "Lookup error asserted before done for index=%0d",
                           current_expected_index);
    ...
    task automatic program_and_check_table_base(
        input logic [63:0] table_base
    );
        logic [31:0] read_low;
        logic [31:0] read_high;
        begin
            axi_lite_write(ADDR_TABLE_BASE_LO, table_base[31:0]);
            axi_lite_write(ADDR_TABLE_BASE_HI, table_base[63:32]);
            axi_lite_read(ADDR_TABLE_BASE_LO, read_low);
            axi_lite_read(ADDR_TABLE_BASE_HI, read_high);
            if ({read_high, read_low} !== table_base)
                $fatal(1,
                       "TABLE_BASE readback mismatch got=%016h expected=%016h",
                       {read_high, read_low}, table_base);
        end
    endtask

    task automatic check_idle_clear(input integer test_index);
        logic [31:0] control_word;
        begin
            axi_lite_read(ADDR_CONTROL, control_word);
    ...
    task automatic run_lookup_case(
        ...
        begin
            ...
            check_idle_clear(test_index);
            pass_count = pass_count + 1;
            $display("PASS case=%0s index=%0d base=%016h address=%016h",
                     case_name, test_index, test_base,
                     current_expected_address);
        end
    endtask

    task automatic run_rejected_case(
        input integer test_index,
        input logic [63:0] rejected_base,
        input string case_name
    );
        integer ar_count_before;
        integer r_count_before;
        ...
        begin
            current_expected_index = test_index;
            current_expected_base = rejected_base;
            current_expected_address = '0;
            ar_count_before = ar_handshake_count;
            r_count_before = r_handshake_count;

            program_and_check_table_base(rejected_base);
            axi_lite_write(ADDR_LOOKUP_INDEX, test_index);
            axi_lite_write(ADDR_CONTROL, 32'h0000_0001);
            wait_for_done(1'b1, control_word);

            axi_lite_read(ADDR_RESULT0, result0);
            axi_lite_read(ADDR_RESULT1, result1);
            axi_lite_read(ADDR_RESULT2, result2);
            axi_lite_read(ADDR_RESULT3, result3);
            if ({result3, result2, result1, result0} !== 128'd0)
                $fatal(1, "%0s did not return a zero vector", case_name);
            if (ar_handshake_count != ar_count_before ||
                r_handshake_count != r_count_before)
                $fatal(1, "%0s caused a physical AXI transaction", case_name);

            check_idle_clear(test_index);
            pass_count = pass_count + 1;
            $display("PASS case=%0s index=%0d base=%016h",
                     case_name, test_index, rejected_base);
        end
    endtask
    ...
    // the DUT must retain a stable 64-bit AR payload under backpressure.
        ...
        if (!ap_rst_n) begin
            ...
            if (m_axi_gmem_arvalid && m_axi_gmem_arready) begin
                ...
                if (m_axi_gmem_araddr[3:0] !== 4'd0)
                    $fatal(1, "Unaligned ARADDR 0x%0h",
                           m_axi_gmem_araddr);
                if (m_axi_gmem_araddr !== current_expected_address)
                    $fatal(1,
                           "Wrong ARADDR index=%0d got=0x%0h expected=0x%0h",
                           current_expected_index,
                           m_axi_gmem_araddr,
                           current_expected_address);

                pending_row <= current_expected_index;
    ...
    initial begin
        ...
        run_lookup_case(0, 64'h0000_0001_0000_4000, "directed_0");
        run_lookup_case(1, 64'h0000_0002_0000_8000, "directed_1");
        run_lookup_case(31, 64'h1234_5678_9ABC_D000, "directed_31");
        run_lookup_case(63, 64'h0000_0001_FFFF_F000, "directed_63");

        random_seed = 32'h14A5_0002;
        ...
        for (random_case = 0; random_case < 10;
            ...
            run_lookup_case(
                random_index,
                64'h0000_0100_0000_0000 + random_case * 64'h1000,
                $sformatf("random_%0d", random_case)
            );
        end

        run_rejected_case(
            7, 64'h0000_0001_0000_4008, "unaligned_base_error");
        run_rejected_case(
            64, 64'h0000_0001_0000_4000, "out_of_range_index_error");
        run_rejected_case(
            1, 64'hFFFF_FFFF_FFFF_FFF0, "address_overflow_error");

        if (pass_count != 17)
            $fatal(1, "Expected 17 passing cases, got %0d", pass_count);
        if (ar_handshake_count != 14 || r_handshake_count != 14)
            $fatal(1,
                   "Expected 14 valid AR/R handshakes, got AR=%0d R=%0d",
