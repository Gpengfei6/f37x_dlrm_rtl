...
module tb_dlrm_f37x_rtl_kernel_stage2n_a18_v1;
    ...
    dlrm_f37x_rtl_kernel_stage2n_a18_v1 dut (
        .ap_clk(ap_clk),
        .ap_rst_n(ap_rst_n),
    ...
    integer programmed_row [0:3];
    integer encoded_enable;

    function automatic integer slot_row(input integer slot);
        begin
            slot_row = programmed_row[slot];
        end
    endfunction

    function automatic logic [ADDR_WIDTH-1:0] index_address(input integer slot);
        begin
            case (slot)
                0: index_address = ADDR_A18_LOOKUP_INDEX0;
                1: index_address = ADDR_A18_LOOKUP_INDEX1;
                2: index_address = ADDR_A18_LOOKUP_INDEX2;
                3: index_address = ADDR_A18_LOOKUP_INDEX3;
                default: index_address = {ADDR_WIDTH{1'bx}};
            endcase
        end
    endfunction

    // Independent golden. Case A uses the A17 identity fixture
    // lane = row*8 + lane - 256 so the sparse A13 net still returns 36.
    // Cases B-H use {bank,row,lane} packing so a wrong slot is obvious.
    function automatic logic [127:0] expected_row(
        input integer bank,
        input integer row
    );
        logic [127:0] packed_data;
        ...
        begin
            ...
            for (lane_i = 0; lane_i < 8; lane_i = lane_i + 1) begin
                if (encoded_enable)
                    lane_value = {bank[3:0], row[5:0], lane_i[5:0]};
                else
                    lane_value = row * 8 + lane_i - 256;
                packed_data[lane_i*16 +: 16] = lane_value;
            end
            expected_row = packed_data;
        end
    endfunction
    ...
    // A17 CLEAR is legal only from DONE or ERROR. Lookup-only cases reset.
    task automatic wait_a15_done;
        begin
            wait_attempts = 0;
            axi_read(ADDR_A15_CONTROL_STATUS, a15_status_value);
            while (a15_status_value[4] !== 1'b1) begin
                wait_attempts = wait_attempts + 1;
                if (wait_attempts > 200000)
                    $fatal(1, "A15 DONE timeout status=0x%08x",
                           a15_status_value);
                axi_read(ADDR_A15_CONTROL_STATUS, a15_status_value);
            end
        end
    ...
    task automatic drive_r(input integer slot, input logic inject_error);
        ...
        begin
            row = slot_row(slot);
            row_data = expected_row(slot, row);
            @(negedge ap_clk);
            ...
            for (lane_i = 0; lane_i < 8; lane_i = lane_i + 1) begin
                ...
                if (m_axi_gmem_rdata[slot][lane_i*16 +: 16] !==
                    held_data[lane_i*16 +: 16])
                    $fatal(1,
                           "bank%0d row%0d lane%0d mismatch actual=%0h expected=%0h",
                           slot, row, lane_i,
                           m_axi_gmem_rdata[slot][lane_i*16 +: 16],
                           held_data[lane_i*16 +: 16]);
            end
            @(negedge ap_clk);
    ...
    task automatic program_lookup_indexes;
        integer slot;
        logic [31:0] readback;
        begin
            for (slot = 0; slot < 4; slot = slot + 1)
                axi_write(index_address(slot), programmed_row[slot]);
            for (slot = 0; slot < 4; slot = slot + 1) begin
                axi_read(index_address(slot), readback);
                if (readback !== programmed_row[slot][31:0])
                    $fatal(1, "index%0d staging mismatch", slot);
            end
        end
    endtask
    ...
    always @(posedge ap_clk) begin
        if (!ap_rst_n)
            ...
            if (cycle_count > 3000000)
                $fatal(1,
                       "tb_dlrm_f37x_rtl_kernel_stage2n_a18_v1: TIMEOUT");
        end
    end
    ...
    initial begin
        ...
        if (version_value !== 32'h0002_4E18)
            $fatal(1, "pipeline version mismatch: 0x%08x", version_value);
        axi_read(ADDR_A18_LOOKUP_INDEX0, status_value);
        axi_read(ADDR_A18_LOOKUP_INDEX1, mask_value);
        axi_read(ADDR_A18_LOOKUP_INDEX2, error_code_value);
        axi_read(ADDR_A18_LOOKUP_INDEX3, reserved_value);
        if ((status_value !== 32'd37) || (mask_value !== 32'd38) ||
            (error_code_value !== 32'd39) || (reserved_value !== 32'd40))
            $fatal(1, "default indexes after reset are not 37,38,39,40");
        $display("A18_1_DEFAULT_INDEX_RESET=PASS");
        axi_read(ADDR_PIPE_EMB_LOADED_MASK, mask_value);
        ...
        $display("A18_1_RESET_CLEAN=PASS");
        $display("A18_1_SATURATION=PASS");

        program_a17_table_bases();
        $display("A18_1_SPLIT_AXI_LITE=PASS");
        $display("A18_1_FOUR_BASE_CAPTURE=PASS");
        ...
        axi_write(ADDR_PIPE_CONTROL_STATUS, PIPE_CMD_ERROR_ACK);
        $display("A18_1_HOST_EMBEDDING_REJECT=PASS");
        ...
        wait_a15_idle();
        $display("A18_1_STALL=PASS");
        $display("A18_1_ERROR=PASS");
        $display("A18_1_ERROR_DRAIN_RECOVERY=PASS");
        ...
        for (run_index = 0; run_index < 2; run_index = run_index + 1) begin
            ...
            $display("A18_1_RUN_%0d=PASS", run_index + 1);
        end

        $display("A18_1_FOUR_AR_BEFORE_ANY_R=PASS");
        $display("A18_1_REORDER=PASS");
        $display("A18_1_COUNTER_RESTART_DETERMINISM=PASS");
        $display("A18_1_RESTART=PASS");
        $display("A18_1_LOADED_MASK=PASS");
        $display("A18_1_RESULT_MATCH=PASS");
        $display("A18_1_A13_COUNTER_ABI=PASS");
        $display("A18_1_CANONICAL_ROWS=37,38,39,40");
        $display("A18_1_GOLDEN_FINAL_RESULT=36");
        $display("A18_1_ACTUAL_FINAL_RESULT=%0d", held_result);
        $display("A18_1_HBM_LOOKUP_CYCLES=%0d",
                 held_hbm_lookup_cycles);
        $display("A18_1_BOTTOM_CYCLES=%0d", held_bottom_cycles);
        $display("A18_1_INTERACTION_CYCLES=%0d",
                 held_interaction_cycles);
        $display("A18_1_TOP_CYCLES=%0d", held_top_cycles);
        $display("A18_1_COMPUTE_TOTAL_CYCLES=%0d", held_total_cycles);
        $display("A18_1_FPGA_END_TO_END_CYCLES=%0d",
                 held_fpga_end_to_end_cycles);
        $display("A18_1_PIPELINE_OVERHEAD_CYCLES=%0d",
                 pipeline_overhead_cycles);
        $display("A18_1_AR_HANDSHAKES=%0d", ar_handshake_count);
        $display("A18_1_R_HANDSHAKES=%0d", r_handshake_count);
        $display("A18_1_FINAL_LOADED_MASK=0x%0h", mask_value[3:0]);
        $display("A18_1_LATENCY_ACCOUNTING=PASS");
        $display("A18_1_CASE_A_DEFAULT_37_40_GOLDEN36=PASS");
        $display("A18_1_NOTE_CASE_A_USES_A17_IDENTITY_FIXTURE_RESULT36");

        // Cases B-H check AXI address/slot/error/status, not the A13 logit.
        encoded_enable = 1;

        // B. four different legal rows 1,2,3,4
        reset_dut();
        programmed_row[0] = 1;
        programmed_row[1] = 2;
        programmed_row[2] = 3;
        ...
        for (step = 0; step < 4; step = step + 1)
            drive_r(reorder_port(step), 1'b0);
        wait_loaded_mask(4'hF);
        $display("A18_1_CASE_B_ROWS_1_2_3_4=PASS");

        // C. first/last row and all-same legal index
        reset_dut();
        programmed_row[0] = 0;
        programmed_row[1] = 63;
        programmed_row[2] = 0;
        ...
        wait_loaded_mask(4'hF);
        $display("A18_1_CASE_C_ROWS_0_63=PASS");
        ...
        wait_loaded_mask(4'hF);
        $display("A18_1_CASE_C_ALL_SEVEN=PASS");

        // D. two consecutive tasks with different indexes
        reset_dut();
        programmed_row[0] = 10;
        programmed_row[1] = 11;
        programmed_row[2] = 12;
        ...
        wait_loaded_mask(4'hF);
        $display("A18_1_CASE_D_TWO_TASKS=PASS");

        // E. busy index write ignored; repeated START; CLEAR restart
        reset_dut();
        programmed_row[0] = 4;
        programmed_row[1] = 5;
        programmed_row[2] = 6;
        ...
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_START);
        wait_arvalid_mask(4'hF);
        axi_write(ADDR_A18_LOOKUP_INDEX0, 32'd50);
        axi_write(ADDR_A18_LOOKUP_INDEX1, 32'd51);
        axi_write(ADDR_A18_LOOKUP_INDEX2, 32'd52);
        axi_write(ADDR_A18_LOOKUP_INDEX3, 32'd53);
        axi_read(ADDR_A18_LOOKUP_INDEX0, status_value);
        axi_read(ADDR_A18_LOOKUP_INDEX1, mask_value);
        axi_read(ADDR_A18_LOOKUP_INDEX2, error_code_value);
        axi_read(ADDR_A18_LOOKUP_INDEX3, reserved_value);
        if ((status_value !== 32'd4) || (mask_value !== 32'd5) ||
            (error_code_value !== 32'd6) || (reserved_value !== 32'd8))
            $fatal(1, "busy index writes were not ignored");
        for (port = 0; port < 4; port = port + 1)
            check_ar_payload(port);
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_START);
        axi_read(ADDR_PIPE_ERROR_CODE, error_code_value);
        if ((error_code_value[17] !== 1'b1) ||
            (error_code_value[11:8] !== 4'd3))
            $fatal(1, "repeated START was not rejected: 0x%08x",
                   error_code_value);
        axi_write(ADDR_PIPE_CONTROL_STATUS, PIPE_CMD_ERROR_ACK);
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_CLEAR);
        axi_read(ADDR_PIPE_ERROR_CODE, error_code_value);
        if ((error_code_value[17] !== 1'b1) ||
            (error_code_value[11:8] !== 4'd3))
            $fatal(1, "mid-sequence CLEAR was accepted: 0x%08x",
                   error_code_value);
        axi_write(ADDR_PIPE_CONTROL_STATUS, PIPE_CMD_ERROR_ACK);
        for (port = 0; port < 4; port = port + 1) begin
            handshake_ar(port);
        ...
        $display("A18_1_CASE_E_BUSY_WRITE_AND_RESTART=PASS");

        // F. R backpressure and complete bank3 before bank0
        reset_dut();
        programmed_row[0] = 14;
        programmed_row[1] = 15;
        programmed_row[2] = 16;
        ...
        drive_r(1, 1'b0);
        drive_r(0, 1'b0);
        if ((response_order_observed[0] !== 3) ||
            (response_order_observed[3] !== 0))
            $fatal(1, "bank3-before-bank0 order mismatch");
        $display("A18_1_CASE_F_BACKPRESSURE_BANK3_FIRST=PASS");
        ...
        axi_read(ADDR_PIPE_EMB_LOADED_MASK, mask_value);
        if (mask_value[3:0] !== 4'h0)
            $fatal(1, "rresp error injected embeddings mask=%h", mask_value[3:0]);
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_CLEAR);
        wait_a15_idle();
        $display("A18_1_CASE_G_RRESP_ERROR=PASS");

        // H. OOB index rejects before any ARVALID; one bad index blocks all four
        reset_dut();
        programmed_row[0] = 64;
        programmed_row[1] = 1;
        programmed_row[2] = 2;
        ...
        repeat (40) begin
            ...
            if (|m_axi_gmem_arvalid)
                $fatal(1, "OOB 64 issued ARVALID=%b", m_axi_gmem_arvalid);
        end
        wait_a15_error();
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_CLEAR);
        wait_a15_idle();
        $display("A18_1_CASE_H_OOB_64=PASS");

        reset_dut();
        programmed_row[0] = 0;
        programmed_row[1] = 1;
        programmed_row[2] = 2;
        programmed_row[3] = 32'hFFFFFFFF;
        program_a17_table_bases();
        program_lookup_indexes();
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_START);
        repeat (40) begin
            @(posedge ap_clk);
            if (|m_axi_gmem_arvalid)
                $fatal(1, "OOB 0xFFFFFFFF issued ARVALID=%b",
                       m_axi_gmem_arvalid);
        end
        wait_a15_error();
        axi_write(ADDR_A15_CONTROL_STATUS, A15_CMD_CLEAR);
        wait_a15_idle();
        $display("A18_1_CASE_H_OOB_FFFFFFFF=PASS");

        $display("A18_1_CASES_B_H_AXI_STATUS_ONLY=PASS");
        $display("STAGE2N_A18_VARIABLE_INDEX_XSIM_V1_PASS");
        $finish;
    end
