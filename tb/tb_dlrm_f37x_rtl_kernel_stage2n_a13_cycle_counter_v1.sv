...
module tb_dlrm_f37x_rtl_kernel_stage2n_a13_cycle_counter_v1;
    ...
    dlrm_f37x_rtl_kernel_stage2n_a13_v1 dut (
        .ap_clk(ap_clk),
        .ap_rst_n(ap_rst_n),
    ...
    task automatic start_pipeline;
        begin
            ...
            // On the second run, prove that accepting START replaced the
            // previous completed total rather than inheriting it.
            if (run_index != 0) begin
                axi_read(ADDR_PIPE_TOTAL_CYCLES, total_cycles_value);
                if ((total_cycles_value == 0) ||
                    (total_cycles_value >= first_total_cycles)) begin
                    $fatal(1,
                           "total counter did not restart: current=%0d prior=%0d",
                           total_cycles_value, first_total_cycles);
                end
            end
        end
    endtask
    ...
    always @(posedge ap_clk) begin
        if (!ap_rst_n)
            ...
            if (cycle_count > 3000000)
                $fatal(1,
                       "tb_dlrm_f37x_rtl_kernel_stage2n_a13_cycle_counter_v1: TIMEOUT");
        end
    end
    ...
    initial begin
        ...
        $display("PIPELINE_WINDOW_VERSION=0x00024e13");
        ...
        $display("BOTTOM_CYCLES=%0d", first_bottom_cycles);
        $display("INTERACTION_CYCLES=%0d", first_interaction_cycles);
        $display("TOP_CYCLES=%0d", first_top_cycles);
        $display("TOTAL_CYCLES=%0d", first_total_cycles);
        $display("CONTROLLER_OVERHEAD_CYCLES=%0d",
                 first_total_cycles - first_bottom_cycles -
                 first_interaction_cycles - first_top_cycles);
        $display("CYCLE_COUNTER_RESTART_MATCH=2");
        $display("tb_dlrm_f37x_rtl_kernel_stage2n_a13_cycle_counter_v1: PASS runs=2 final=36 descriptors=5 weights=1360 biases=73");
        $finish;
    end
